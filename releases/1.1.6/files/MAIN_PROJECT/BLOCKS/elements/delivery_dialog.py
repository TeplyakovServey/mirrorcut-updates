...
def _mirror_cut_root() -> str:
    """Корень репозитория MIRROR_CUT (elements → … → FINAL_WINDOW → repo)."""
...
def _cities_db_path() -> str:
    ...
    for p in (
        os.path.join(root, "CALC_WINDOWS", "cities_streets.db"),
        os.path.join(root, "FINAL_WINDOW", "CALC_WINDOWS", "cities_streets.db"),
        os.path.join(main_project, "CALC_WINDOWS", "cities_streets.db"),
    ):
        if os.path.isfile(p):
            return p
    return os.path.join(root, "CALC_WINDOWS", "cities_streets.db")
...
class DeliveryOutsideDialog(QDialog):
    ...
    def __init__(
        ...
        row2 = QHBoxLayout()
        b_rr = QPushButton("Перестроить маршрут")
        b_rr.clicked.connect(self._on_route_force)
        row2.addWidget(b_rr)
    ...
    def _refresh_delivery_price(self):
        ...
        try:
            ...
            if not conn:
                self._price_lbl.setText("Цена: нет связи с БД")
                return
            prices = fetch_delivery_prices(conn=conn)
            km = None
    ...
    def _run_geocode(self, q: str):
        g = geocode_by_token(q.strip(), self._token)
        if not g:
        ...
        if self._map:
            self._map.reload_map(self._lat, self._lon, None)
    ...
    def _build_route(self):
        ...
        if not res:
            self._status.setText(
                "Маршрут не построен. Проверьте сеть или введите км вручную."
            )
            self._route_coords = None
            self._distance_m = None
            self._distance_km = None
