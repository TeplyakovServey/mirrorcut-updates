# -*- coding: utf-8 -*-
"""Диалог управления ценами: «Фасады», «Работы», «Стекло / зеркало» (PostgreSQL калькулятора)."""
import sys
import os
import math
...
from ui.glass_prices_tab import GlassPricesTab, WORKS_PRICE_SECTIONS
...
class PricesDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        self._works_prices_tab = GlassPricesTab(self, sections=WORKS_PRICE_SECTIONS)
        tabs.addTab(self._works_prices_tab, "Работы")
    ...
    def _is_facades_tab(self) -> bool:
        w = self._tabs.currentWidget()
        return w not in (
            getattr(self, "_glass_prices_tab", None),
            getattr(self, "_works_prices_tab", None),
        )

    def _sync_profiles_excel_bar(self):
        """Демо/импорт Excel только для админа и только на подвкладке «Профили»."""
        show = (
            self._is_facades_tab()
    ...
    def _on_prices_tab_changed(self, index: int):
        """На вкладках стекла и работ скрываем кнопки, относящиеся только к фасадам."""
        is_facades = self._is_facades_tab()
        for ob in getattr(self, "_facades_only_widgets", ()):
            ob.setVisible(is_facades)
