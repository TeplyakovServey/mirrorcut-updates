...
def _piece_p400_label(p: Dict[str, Any], ctx: GExportContext) -> str:
    """
    Подпись изделия в P400: __K5 (как на этикетке), если у заказа K-нумерация;
    иначе __5. Мелкие детали — __*N (как *N на схеме).
    """
    ...
    ctx.piece_counter += 1
    if ctx.k_base is not None:
        disp = int(ctx.k_base) + piece_num - 1
        return "__K%d" % disp
    return "__%d" % piece_num


def _format_p400_tail(label: str = "", number: Optional[int] = None, *, piece_label: str = "") -> str:
    """
    Хвост P400 как в эталоне Optima:
    - изделие: __K5 или __5
    - остаток: __13
    - отход: _ОТХОД_ + 32 подчёркивания (без номера)
    """
    if piece_label:
        tail = piece_label
    elif label and number is None:
        tail = "_%s_" % label.strip()
    elif label:
        tail = "_%s_%d" % (label.strip(), max(1, _safe_int(number, 1)))
    else:
...
def _format_p400_zone(
    ...
    *,
    ...
    return core + _format_p400_tail(label, number, piece_label=piece_label)
...
def _build_p400_zones(
    ...
    for idx in _sorted_piece_indices(pieces):
        ...
        num = _piece_p400_label(p, ctx)
        zones.append(_format_p400_zone(x, y, w, h, sh, piece_label=num))
