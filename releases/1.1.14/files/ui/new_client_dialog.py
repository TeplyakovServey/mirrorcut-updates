...
class NewClientDialog(QDialog):
    ...
    def __init__(
        ...
        self.okpo_edit.setMaxLength(10)
        self.okpo_edit.setValidator(_digits_validator(10))
        self.doc_ip_layout.addRow("ОКПО (10 цифр, обяз.):", self.okpo_edit)
        self.ogrn_edit = QLineEdit()
        self.ogrn_edit.setPlaceholderText("15 цифр (ОГРНИП)")
        self.ogrn_edit.setMaxLength(15)
        self.ogrn_edit.setValidator(_digits_validator(15))
        self.doc_ip_layout.addRow("ОГРНИП (15 цифр, обяз.):", self.ogrn_edit)
        ...
        self.doc_ind_layout.addRow("Паспорт, серия (необяз.):", self.passport_series_edit)
        ...
        self.doc_ind_layout.addRow("Паспорт, номер (необяз.):", self.passport_number_edit)
    ...
    def _on_type_changed(self):
        ...
        if t == TYPE_LEGAL:
            ...
            self.bank_grp.setVisible(False)
    ...
    def _load_client(self, c):
        ...
        self.ogrn_edit.setText(str(c.get('ogrnip') or c.get('ogrn') or '') if (c.get('ogrnip') or c.get('ogrn')) else '')
    ...
    def _validate_ip(self):
        ...
        if len(kpp) != KPP_LEN:
            QMessageBox.warning(self, "Ошибка", "КПП — 9 цифр.")
            return False
        if len(okpo) != OKPO_LEN_IP:
            QMessageBox.warning(self, "Ошибка", "ОКПО ИП — 10 цифр.")
            return False
        if len(ogrn) != OGRNIP_LEN:
            QMessageBox.warning(self, "Ошибка", "ОГРНИП — 15 цифр.")
            return False
        if not self._phone_valid():
            QMessageBox.warning(
                self,
    ...
    def _validate_individual(self):
        ...
        if inn and not _inn_valid(inn):
            ...
            return False
        if passport_s and (len(passport_s) != 4 or not passport_s.isdigit()):
            QMessageBox.warning(self, "Ошибка", "Серия паспорта — 4 цифры или пусто.")
            return False
        if passport_n and (len(passport_n) != 6 or not passport_n.isdigit()):
            QMessageBox.warning(self, "Ошибка", "Номер паспорта — 6 цифр или пусто.")
            return False
        if (passport_s and not passport_n) or (passport_n and not passport_s):
            QMessageBox.warning(self, "Ошибка", "Укажите серию и номер паспорта вместе или оставьте оба поля пустыми.")
            return False
        if not self._phone_valid():
            QMessageBox.warning(
                self,
    ...
    def _save(self):
        ...
        ogrn_val = (self.ogrn_edit.text() or "").strip() or None
        ogrnip_val = ogrn_val if t == TYPE_IP else None
        if t == TYPE_IP:
            ogrn_val = None
        bank_account = (self.bank_account_edit.text() or "").strip() or None if t != TYPE_INDIVIDUAL else None
        corr_account = (self.corr_account_edit.text() or "").strip() or None if t != TYPE_INDIVIDUAL else None
        bank_name = (self.bank_name_edit.text() or "").strip() or None if t != TYPE_INDIVIDUAL else None
        bik = (self.bik_edit.text() or "").strip() or None if t != TYPE_INDIVIDUAL else None
        if self._is_supplier:
            bank_account = (self.bank_account_edit.text() or "").strip() or None
            corr_account = (self.corr_account_edit.text() or "").strip() or None
            bank_name = (self.bank_name_edit.text() or "").strip() or None
            bik = (self.bik_edit.text() or "").strip() or None
        ...
        try:
            if self._is_supplier:
                ...
                models.update_client_full(
                    self.edit_client['id'], t, name,
                    inn or None, kpp or None, okpo, ogrn_val, registration,
                    first_name, last_name, passport_series, passport_number, birth_date, gender,
                    phone, email, legal_address, actual_address, source, notes, pricing_tier,
                    ogrnip=ogrnip_val,
                    bank_account=bank_account, corr_account=corr_account, bank_name=bank_name, bik=bik,
                )
            else:
                new_id = models.insert_client_full(
                    t, name, inn or None, kpp or None, okpo, ogrn_val, registration,
                    first_name, last_name, passport_series, passport_number, birth_date, gender,
                    phone, email, legal_address, actual_address, source, notes, pricing_tier,
                    ogrnip=ogrnip_val,
                    bank_account=bank_account, corr_account=corr_account, bank_name=bank_name, bik=bik,
