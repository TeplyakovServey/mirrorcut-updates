...
def _ensure_profile_stock_tables():
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                """
                CREATE TABLE IF NOT EXISTS mirror_deleted_full_sheets (
                    id SERIAL PRIMARY KEY,
                    sheet_id INTEGER,
                    name VARCHAR(255) NOT NULL DEFAULT '',
            ...
            for col, typ in (
                ...
                cur.execute(
                    ...
                    WHERE table_name = 'mirror_deleted_profile_stock' AND column_name = %s
                    """,
                    (col,),
                )
                if not cur.fetchone():
                    cur.execute(
                        "ALTER TABLE mirror_deleted_profile_stock ADD COLUMN %s %s"
                        % (col, typ)
                    )
...
def delete_profile_stock_and_archive(stock_id, deleted_by_login=None, deleted_by_display=None):
    _ensure_profile_stock_tables()
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
...
def get_deleted_profile_stock(limit=500):
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """SELECT id, stock_id, item_type, ref_id, series, name, color, length_mm, quantity, is_remnant,
                          deleted_at, deleted_by_login, deleted_by_display
...
# --- Full sheets ---
FULL_SHEET_SELECT = (
    "id, name, height_mm, width_mm, thickness_mm, arrival_date, invoice_date, supplier_id, supplier, "
    "cost, warehouse_number, quantity, comment"
)
...
def get_all_full_sheets():
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT " + FULL_SHEET_SELECT + " FROM mirror_full_sheets ORDER BY name")
            return cur.fetchall()
...
def get_full_sheets_by_name_prefix(prefix):
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT " + FULL_SHEET_SELECT + " FROM mirror_full_sheets WHERE name ILIKE %s AND quantity > 0 ORDER BY name",
                (prefix + '%',),
            )
            return cur.fetchall()
...
def get_full_sheet_by_id(sheet_id):
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT " + FULL_SHEET_SELECT + " FROM mirror_full_sheets WHERE id = %s",
                (sheet_id,),
            )
            return cur.fetchone()


def insert_full_sheet(
    name,
    height_mm,
    width_mm,
    arrival_date=None,
    ...
    if supplier_id and not (supplier or "").strip():
        row = get_supplier_by_id(supplier_id)
        if row:
            supplier = _supplier_display_name(row)
    supplier = (supplier or "").strip() or None
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """INSERT INTO mirror_full_sheets (
                    name, height_mm, width_mm, thickness_mm, arrival_date, invoice_date, supplier_id, supplier,
                    cost, warehouse_number, quantity, comment
...
def insert_deleted_full_sheet_archive(
    name,
    height_mm,
    width_mm,
    thickness_mm=4,
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """INSERT INTO mirror_deleted_full_sheets
                   (sheet_id, name, height_mm, width_mm, thickness_mm, quantity, supplier, cost,
                    warehouse_number, deleted_by_login, deleted_by_display)
                   VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)""",
                (
                    sheet_id,
                    name or "",
...
def get_deleted_full_sheets(limit=500):
    _ensure_profile_stock_tables()
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """SELECT id, sheet_id, name, height_mm, width_mm, thickness_mm, quantity, supplier, cost,
                          warehouse_number, deleted_at, deleted_by_login, deleted_by_display
                   FROM mirror_deleted_full_sheets ORDER BY deleted_at DESC LIMIT %s""",
                (int(limit),),
            )
            return cur.fetchall()


def delete_full_sheet_and_archive(sheet_id, deleted_by_login=None, deleted_by_display=None):
    """Скопировать целый лист в архив удалённых, затем удалить из mirror_full_sheets."""
    row = get_full_sheet_by_id(sheet_id)
    if not row:
        return False
    insert_deleted_full_sheet_archive(
        row.get("name") or "",
        row.get("height_mm") or 0,
        row.get("width_mm") or 0,
...
# --- Clients ---
CLIENT_SELECT = """id, client_type, name, inn, kpp, okpo, ogrn, ogrnip, registration,
...
def _client_search_prefix_variants(prefix):
    """EN QWERTY и как введено — для поиска клиента при неверной раскладке."""
    raw = (prefix or '').strip()
    if not raw:
        return ()
    try:
...
def get_clients_by_prefix(prefix):
    prefs = _client_search_prefix_variants(prefix)
    if not prefs:
        return _get_clients_cached()
    seen = set()
    out = []
    for pref in prefs:
        for r in _get_clients_cached():
            ...
            if hay.startswith(pref) or any(t.startswith(pref) for t in tokens) or pref in hay:
                seen.add(rid)
                out.append(r)
    return out
...
def get_clients_search(query):
    ...
    variants = _client_search_prefix_variants(query)
    if not variants:
        return []
    seen = set()
    out = []
...
def get_client_id_by_name(name):
    ...
    if not nm:
        return None
    candidates = [nm]
    for v in _client_search_prefix_variants(nm):
        if v not in candidates:
            candidates.append(v)
    for cand in candidates:
        nm_l = cand.strip().lower()
...
def get_mirror_quick_client_id_by_name(name):
    if not (name or "").strip():
        return None
    candidates = [(name or "").strip()]
    for v in _client_search_prefix_variants(name):
        vv = (v or "").strip()
...
def insert_client_full(
    ...
    ogrnip=None, bank_account=None, corr_account=None, bank_name=None, bik=None,
):
        ...
        with conn.cursor() as cur:
            cur.execute(
                """INSERT INTO mirror_clients (
                    client_type, name, inn, kpp, okpo, ogrn, ogrnip, registration,
                    first_name, last_name, passport_series, passport_number, birth_date, gender,
                ...
                (
                    ...
                    (okpo or '').strip() or None, (ogrn or '').strip() or None,
                    (ogrnip or '').strip() or None, (registration or '').strip() or None,
                    ...
                    (bank_account or '').strip() or None,
                    (corr_account or '').strip() or None,
                    (bank_name or '').strip() or None,
                    (bik or '').strip() or None,
                )
            )
            cid = cur.fetchone()['id']
        ...
        with conn.cursor() as cur:
            cur.execute(
                """UPDATE mirror_clients SET
                    client_type=%s, name=%s, inn=%s, kpp=%s, okpo=%s, ogrn=%s, ogrnip=%s,
                    registration=%s, first_name=%s, last_name=%s, passport_series=%s, passport_number=%s,
                    birth_date=%s, gender=%s, phone=%s, email=%s, legal_address=%s, actual_address=%s,
                    source=%s, notes=%s, pricing_tier=%s, bank_account=%s, corr_account=%s, bank_name=%s, bik=%s
                    WHERE id=%s""",
                (
...
ENTITY_SELECT = """id, owner_client_id, entity_type, display_name, inn, kpp, okpo,
    ogrn, ogrnip, bank_account, corr_account, bank_name, bik, is_default, created_at"""


def list_client_entities(owner_client_id):
    try:
        oid = int(owner_client_id)
    except (TypeError, ValueError):
        return []
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT " + ENTITY_SELECT.replace('\n', ' ').strip()
                + " FROM mirror_client_entities WHERE owner_client_id = %s ORDER BY is_default DESC, display_name",
                (oid,),
            )
            return list(cur.fetchall() or [])


def get_client_entity_by_id(entity_id):
    try:
        eid = int(entity_id)
    except (TypeError, ValueError):
        return None
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT " + ENTITY_SELECT.replace('\n', ' ').strip()
                + " FROM mirror_client_entities WHERE id = %s",
                (eid,),
            )
            return cur.fetchone()


def save_client_entity(owner_client_id, data: dict, entity_id=None):
    try:
        oid = int(owner_client_id)
    except (TypeError, ValueError):
        return None
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            vals = (
                oid,
                (d.get('entity_type') or 'legal').strip(),
                (d.get('display_name') or '').strip() or '—',
                (d.get('inn') or '').strip() or None,
                (d.get('kpp') or '').strip() or None,
                (d.get('okpo') or '').strip() or None,
                (d.get('ogrn') or '').strip() or None,
                (d.get('ogrnip') or '').strip() or None,
                (d.get('bank_account') or '').strip() or None,
                (d.get('corr_account') or '').strip() or None,
                (d.get('bank_name') or '').strip() or None,
                (d.get('bik') or '').strip() or None,
                bool(d.get('is_default')),
            )
            if entity_id:
                cur.execute(
                    """UPDATE mirror_client_entities SET
                        entity_type=%s, display_name=%s, inn=%s, kpp=%s, okpo=%s,
                        ogrn=%s, ogrnip=%s, bank_account=%s, corr_account=%s, bank_name=%s, bik=%s,
                        is_default=%s WHERE id=%s AND owner_client_id=%s RETURNING id""",
                    vals[1:] + (int(entity_id), oid),
                )
            else:
                cur.execute(
                    """INSERT INTO mirror_client_entities (
                        owner_client_id, entity_type, display_name, inn, kpp, okpo,
                        ogrn, ogrnip, bank_account, corr_account, bank_name, bik, is_default
...
def delete_client_entity(entity_id, owner_client_id):
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "DELETE FROM mirror_client_entities WHERE id = %s AND owner_client_id = %s",
                (int(entity_id), int(owner_client_id)),
            )


# --- Suppliers ---
SUPPLIER_SELECT = """id, supplier_type, name, inn, kpp, okpo, ogrn, registration,
    first_name, last_name, passport_series, passport_number, birth_date, gender,
    phone, email, legal_address, actual_address, source, notes, registration_date,
    bank_account, corr_account, bank_name, bik"""
...
def _supplier_columns():
    return SUPPLIER_SELECT.replace('\n', ' ').strip()


_SUPPLIERS_CACHE_TTL_SEC = 15.0
_suppliers_cache_rows = None
_suppliers_cache_ts = 0.0


def _invalidate_suppliers_cache() -> None:
    global _suppliers_cache_rows, _suppliers_cache_ts
    _suppliers_cache_rows = None
    _suppliers_cache_ts = 0.0


def _get_suppliers_cached(force: bool = False):
    global _suppliers_cache_rows, _suppliers_cache_ts
    now = time.time()
    if (not force) and _suppliers_cache_rows is not None and (now - _suppliers_cache_ts) <= _SUPPLIERS_CACHE_TTL_SEC:
        return list(_suppliers_cache_rows)
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT " + _supplier_columns() + " FROM mirror_suppliers ORDER BY name")
            rows = cur.fetchall() or []
    _suppliers_cache_rows = list(rows)
    _suppliers_cache_ts = now
    return list(rows)
...
def get_supplier_by_id(supplier_id):
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT " + _supplier_columns() + " FROM mirror_suppliers WHERE id = %s",
                (int(supplier_id),),
            )
            return cur.fetchone()
...
def _supplier_display_name(row):
    if not row:
        return ""
    if row.get('supplier_type') == 'individual' and (row.get('first_name') or row.get('last_name')):
        return ('%s %s' % (row.get('last_name') or '', row.get('first_name') or '')).strip() or row.get('name') or ''
    return row.get('name') or ''


def is_unknown_supplier_row(row):
    if not row:
        return False
    nm = _norm_supplier_name(_supplier_display_name(row) or row.get('name') or '')
    return nm == _norm_supplier_name(UNKNOWN_SUPPLIER_NAME)


def get_unknown_supplier_id():
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
...
def get_full_sheet_receipt_lines(name, thickness_mm=None, limit=500):
    """Все строки поступлений по материалу (для drill-down со склада)."""
    nm = (name or "").strip()
    with get_connection() as conn:
        with conn.cursor() as cur:
            q = (
                "SELECT " + FULL_SHEET_SELECT + " FROM mirror_full_sheets "
                "WHERE TRIM(name) = %s AND quantity > 0"
            )
            params = [nm]
            if thickness_mm is not None:
                q += " AND thickness_mm = %s"
                params.append(int(thickness_mm))
            q += " ORDER BY arrival_date DESC NULLS LAST, id DESC LIMIT %s"
            params.append(int(limit))
            cur.execute(q, tuple(params))
            return cur.fetchall() or []


def insert_supplier_full(
    supplier_type, name,
    inn, kpp, okpo, ogrn, registration,
    first_name, last_name, passport_series, passport_number, birth_date, gender,
    phone, email, legal_address, actual_address, source=None, notes=None,
    bank_account=None, corr_account=None, bank_name=None, bik=None,
):
    if supplier_type == 'individual' and not (name or '').strip() and (first_name or last_name):
        name = ('%s %s' % ((last_name or '').strip(), (first_name or '').strip())).strip()
    name = (name or '').strip() or '—'
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """INSERT INTO mirror_suppliers (
                    supplier_type, name, inn, kpp, okpo, ogrn, registration,
                    first_name, last_name, passport_series, passport_number, birth_date, gender,
                    phone, email, legal_address, actual_address, source, notes,
                    bank_account, corr_account, bank_name, bik
                ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING id""",
                (
                    (supplier_type or 'legal'),
                    name,
                    (inn or '').strip() or None, (kpp or '').strip() or None,
                    ...
                    (legal_address or '').strip(), (actual_address or '').strip(),
                    (source or '').strip() or None, (notes or '').strip() or None,
                    (bank_account or '').strip() or None, (corr_account or '').strip() or None,
                    (bank_name or '').strip() or None, (bik or '').strip() or None,
                ),
            )
            sid = cur.fetchone()['id']
            _invalidate_suppliers_cache()
...
def update_supplier_full(
    supplier_id, supplier_type, name,
    inn, kpp, okpo, ogrn, registration,
    first_name, last_name, passport_series, passport_number, birth_date, gender,
    phone, email, legal_address, actual_address, source=None, notes=None,
    bank_account=None, corr_account=None, bank_name=None, bik=None,
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """UPDATE mirror_suppliers SET
                    supplier_type=%s, name=%s, inn=%s, kpp=%s, okpo=%s, ogrn=%s, registration=%s,
                    first_name=%s, last_name=%s, passport_series=%s, passport_number=%s, birth_date=%s, gender=%s,
                    phone=%s, email=%s, legal_address=%s, actual_address=%s, source=%s, notes=%s,
                    bank_account=%s, corr_account=%s, bank_name=%s, bik=%s WHERE id=%s""",
                (
                    supplier_type or 'legal', name,
                    (inn or '').strip() or None, (kpp or '').strip() or None,
                    (okpo or '').strip() or None, (ogrn or '').strip() or None, (registration or '').strip() or None,
                    (first_name or '').strip() or None, (last_name or '').strip() or None,
                    (passport_series or '').strip() or None, (passport_number or '').strip() or None,
...
def get_supplier_deliveries(supplier_id, limit=25):
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """SELECT id, name, height_mm, width_mm, thickness_mm, arrival_date, invoice_date, supplier, supplier_id,
                          cost, warehouse_number, quantity, comment
                   FROM mirror_full_sheets
                   WHERE supplier_id = %s
                   ORDER BY arrival_date DESC NULLS LAST, id DESC
                   LIMIT %s""",
                (int(supplier_id), int(limit)),
            )
            return cur.fetchall() or []


def get_supplier_stats_aggregate(supplier_id, date_from=None, date_to_exclusive=None):
    """Агрегаты для статистики поставщика за период (date_to_exclusive — не включая)."""
    where = [
        "supplier_id = %s",
        "COALESCE(height_mm, 0) > 0",
        "COALESCE(width_mm, 0) > 0",
    ]
    params = [int(supplier_id)]
    if date_from is not None:
        where.append("arrival_date IS NOT NULL AND arrival_date >= %s")
    ...
    sql = """
        SELECT
            ...
            COALESCE(SUM(quantity), 0)::int AS sheets_qty,
            COALESCE(SUM((height_mm::numeric * width_mm::numeric / 1000000.0) * quantity), 0) AS area_m2,
            COALESCE(SUM(cost * quantity), 0) AS purchase_rub,
            COUNT(DISTINCT name)::int AS materials_count,
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                """
                SELECT name,
                       SUM((height_mm::numeric * width_mm::numeric / 1000000.0) * quantity) AS area
                FROM mirror_full_sheets
                WHERE """ + " AND ".join(where) + """
                GROUP BY name ORDER BY area DESC LIMIT 1
                """,
                tuple(params),
            )
            top = cur.fetchone()
    area = float(base.get('area_m2') or 0)
    purchase = float(base.get('purchase_rub') or 0)
    top_area = float((top or {}).get('area') or 0)
    last_arrival = base.get('last_arrival')
    days_since_last = None
    if last_arrival is not None:
        try:
            from datetime import date as _date_cls, datetime as _dt_cls

            if isinstance(last_arrival, _dt_cls):
                d_last = last_arrival.date()
            elif hasattr(last_arrival, 'year'):
                d_last = last_arrival
            else:
                d_last = None
            if d_last is not None:
                days_since_last = (_date_cls.today() - d_last).days
        except Exception:
            days_since_last = None
    return {
        'deliveries_count': int(base.get('deliveries_count') or 0),
        'sheets_qty': int(base.get('sheets_qty') or 0),
        'area_m2': area,
        'purchase_rub': purchase,
        'price_per_m2': (purchase / area) if area > 0 else 0.0,
        'materials_count': int(base.get('materials_count') or 0),
        'avg_sheet_m2': (area / int(base.get('sheets_qty') or 1)) if int(base.get('sheets_qty') or 0) > 0 else 0.0,
        'last_arrival': last_arrival,
        'days_since_last': days_since_last,
        'top_material': (top or {}).get('name'),
        'top_material_share_pct': (100.0 * top_area / area) if area > 0 and top_area else 0.0,
    }


def delete_supplier(supplier_id):
    """Удалить поставщика; на листах снимаем supplier_id и текст supplier (чтобы не воскресал при миграции)."""
    sid = int(supplier_id)
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                SET supplier_id = NULL, supplier = NULL
                WHERE supplier_id = %s
                """,
                (sid,),
            )
            cur.execute("DELETE FROM mirror_suppliers WHERE id = %s", (sid,))
            if cur.rowcount == 0:
                raise ValueError("Поставщик не найден (id=%s)" % sid)
    _invalidate_suppliers_cache()
...
# Заказы с этими статусами не дают листов в пул «в работе» для нового раскроя.
_ORDER_STATUSES_BLOCKED_FOR_IN_WORK_POOL = frozenset(
    ('made', 'completed', 'shipped', 'cancelled', 'closed')
)
...
def get_order_for_labels(order_id):
        ...
        with conn.cursor() as cur:
            cur.execute(
                """SELECT o.id, o.client_id, o.client_name AS o_client_name, o.created_at, o.status, o.accepted_at, o.notes, o.k_number,
                          COALESCE(c.name, o.client_name, '') AS client_name,
                          c.client_type, c.first_name, c.last_name, c.name AS client_ref_name
                   FROM mirror_orders o
                   LEFT JOIN mirror_clients c ON c.id = o.client_id
                   WHERE o.id = %s""",
                (order_id,)
    ...
    if not row:
        return None
    row = dict(row)
    display = _client_display_name(row)
    if display:
        row['client_name'] = display
    return row
...
def get_orders_list_revision():
    """Лёгкий отпечаток списка заказов для live-sync главной таблицы."""
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT COUNT(*)::bigint AS cnt,
                       ...
                       COALESCE(MAX(updated_at), TIMESTAMP '1970-01-01') AS max_u
                FROM mirror_orders
                """
            )
            mo = cur.fetchone() or {}
            ...
            try:
                cur.execute(
                    ...
                    SELECT COUNT(*)::bigint AS cnt,
                           COALESCE(MAX(updated_at), TIMESTAMP '1970-01-01') AS max_u
                    FROM mirror_sales_orders
                    """
                )
                sr = cur.fetchone() or {}
                sales_cnt = int(sr.get("cnt") or 0)
                sales_max_u = sr.get("max_u")
            except Exception:
                pass
            cur.execute(
                """
    ...
    return (
        int(mo.get("cnt") or 0),
        int(mo.get("max_id") or 0),
        str(max_u) if max_u is not None else "",
        int(sales_cnt),
        str(sales_max_u) if sales_max_u is not None else "",
        int(cl.get("cnt") or 0),
        str(cl_u) if cl_u is not None else "",
    )
...
def in_work_cut_sheet_eligible_for_pool(order_id, sheet_index, order_row=None):
    """
    Можно ли использовать лист «в работе» в новой сессии раскроя (дозаполнение / пустой лист).
    Нет: заказ завершён/отгружен/отменён; по листу в производстве отмечен «рез выполнен».
    """
    try:
        oid = int(order_id)
        si0 = int(sheet_index)
    ...
    if oid < 1 or si0 < 0:
        return False
    orow = order_row if isinstance(order_row, dict) else get_order(oid)
    if not orow:
        return False
    st = (orow.get('status') or '').strip().lower()
    if st in _ORDER_STATUSES_BLOCKED_FOR_IN_WORK_POOL:
        return False
    if st not in tuple(s.lower() for s in _ORDER_STATUSES_ACTIVE_FOR_CUT):
        return False
    sheet_done = _cut_sheet_done_by_index_for_order(oid)
    if sheet_done.get(si0 + 1):
        return False
    return True
...
def get_sheets_in_work_for_material_thickness(material_name, thickness_mm, min_rect_side=20):
    ...
    for o in orders:
        ...
        for sheet_index, cr in enumerate(rows):
            if not in_work_cut_sheet_eligible_for_pool(order_id, sheet_index, o):
                continue
...
def sync_order_status_made_if_all_products_complete(order_id):
    ...
    if cur_st in ("made", "shipped", "completed"):
        return False
    try:
        from MAIN_PROJECT.logic.blocks_bundle import parse_bundle
    except Exception:
    ...
    done = frozenset({"made", "shipped", "completed"})
    facade_events = None
...
SALES_STATUS_IN_PROGRESS = 'in_progress'
SALES_STATUS_ASSEMBLY = 'assembly'
...
SALES_STATUS_RU = {
    ...
    SALES_STATUS_IN_PROGRESS: 'В работе',
    SALES_STATUS_ASSEMBLY: 'Сборка',
...
def sales_order_needs_cut_profile(items):
    """Профиль «на отрез» (м) — нужен статус «В работе» и резка."""
    for it in items or []:
        if str((it or {}).get('item_type') or '').strip().lower() != 'profile':
            continue
        if str((it or {}).get('unit') or 'pcs').strip().lower() == 'm':
            return True
    return False


def sales_status_flow_for_items(items):
    """
    Без отреза профиля (м): просчитан → оплачен → сборка → собран → …
    С отрезом: … → оплачен → в работе → сборка → собран → …
    «В работе» в списке только если есть профиль на метры.
    """
    flow = [SALES_STATUS_CALCULATED, SALES_STATUS_PAID]
    if sales_order_needs_cut_profile(items):
        flow.append(SALES_STATUS_IN_PROGRESS)
    flow.extend(
        [
            SALES_STATUS_ASSEMBLY,
            SALES_STATUS_ASSEMBLED,
...
def sales_default_status_for_new_order(items):
    """Статус при первом сохранении новой продажи."""
    if sales_order_needs_cut_profile(items):
        return SALES_STATUS_CALCULATED
    return SALES_STATUS_ASSEMBLY
...
def _sales_profile_units_already_reserved(cur, order_id, unit):
    cur.execute(
        """SELECT 1 FROM mirror_sales_profile_usage u
           JOIN mirror_sales_items i ON i.id = u.sales_item_id
           WHERE u.sales_order_id = %s AND i.unit = %s
           LIMIT 1""",
        (int(order_id), str(unit).strip().lower()),
    )
    return cur.fetchone() is not None


def _sales_consume_profiles_for_units(cur, order_row, units):
    """Списание профиля со склада только для указанных unit ('pcs' хлыст 6 м, 'm' отрез 1 м)."""
    oid = int(order_row.get('id'))
    want = {str(u).strip().lower() for u in (units or [])}
    if not want:
        return
    cur.execute(
    ...
    for it in items:
        ...
        for _n in range(qty):
            ...
            if reserved is None:
                if unit == 'm':
                    raise ValueError(
                        "Недостаточно профиля на складе для перевода в «В работе» (отрез по метрам)."
                    )
                raise ValueError(
                    "Недостаточно профиля на складе для статуса «Собран» (целые хлысты 6 м)."
                )
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            if current == ns:
                return get_sales_order(oid)
            cur.execute(
                "SELECT item_type, unit FROM mirror_sales_items WHERE sales_order_id = %s",
                (oid,),
            )
            order_items = [dict(r) for r in (cur.fetchall() or [])]
            flow = sales_status_flow_for_items(order_items)
            if current in flow and ns in flow:
                if flow.index(ns) < flow.index(current):
                    raise ValueError("Нельзя переводить заказ продажи в предыдущий статус.")
                if flow.index(ns) > flow.index(current) + 1:
                    raise ValueError("Переход только на следующий статус по цепочке.")
            if ns not in flow:
                raise ValueError("Недопустимый переход статуса для состава этого заказа.")
            if ns == SALES_STATUS_ASSEMBLED and current == SALES_STATUS_PAID:
                if SALES_STATUS_IN_PROGRESS in flow:
                    raise ValueError(
                        "Сначала переведите заказ в статус «В работе» (профиль на отрез)."
                    )
                if SALES_STATUS_ASSEMBLY in flow:
                    raise ValueError("Сначала переведите заказ в статус «Сборка».")
            if (
                ns == SALES_STATUS_ASSEMBLED
                and current == SALES_STATUS_IN_PROGRESS
                and SALES_STATUS_ASSEMBLY in flow
            ):
                raise ValueError("Сначала переведите заказ в статус «Сборка».")
            if (
                ns == SALES_STATUS_IN_PROGRESS
                and SALES_STATUS_IN_PROGRESS not in flow
            ):
                raise ValueError(
                    "Статус «В работе» недоступен: в заказе нет профиля на отрез (метры)."
                )
            # «Оплачен» — без проверки склада; «В работе» — отрез (м); «Собран» — хлысты (шт).
            if ns == SALES_STATUS_IN_PROGRESS and current != SALES_STATUS_IN_PROGRESS:
                if sales_order_needs_cut_profile(order_items):
                    if not _sales_profile_units_already_reserved(cur, oid, 'm'):
                        _sales_consume_profiles_for_units(cur, row, ('m',))
            if ns == SALES_STATUS_ASSEMBLED and current != SALES_STATUS_ASSEMBLED:
                if not _sales_profile_units_already_reserved(cur, oid, 'pcs'):
                    _sales_consume_profiles_for_units(cur, row, ('pcs',))
            cur.execute(
