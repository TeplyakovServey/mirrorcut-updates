...
class MainApp(QWidget):
    ...
    def __init__(
        ...
        *,
        ...
        embedded_in_dialog: bool = False,
        facade_pick_mode: bool = False,
        shared_pricing_cache: Optional[dict] = None,
        defer_pricing_warm: bool = False,
    ):
        super().__init__()
        self.elements = elements if elements is not None else {}
        self._embedded_in_dialog = bool(embedded_in_dialog)
        self._facade_pick_mode = bool(facade_pick_mode)
        ...
        if not self._embedded_in_dialog:
        ...
        self.glass = GlassProductTile(
            show_additional_button=show_glass_additional_button,
            compact_height=self._facade_pick_mode,
        )
        ...
        self.client_strip.clientIdentityChanged.connect(self._schedule_glass_material_preview)
        self.client_strip.clientIdentityChanged.connect(self._on_client_identity_changed)

        self._glass_preview_timer = QTimer(self)
        self._glass_preview_timer.setSingleShot(True)
        self._glass_preview_timer.setInterval(120)
        self._glass_preview_timer.timeout.connect(self._refresh_glass_material_preview)
        ...
        self._local_metrics_timer.setSingleShot(True)
        self._local_metrics_timer.setInterval(100)
        self._local_metrics_timer.timeout.connect(self._on_glass_local_metrics_impl)

        self.initUI()
        if shared_pricing_cache is not None:
            self._pricing_cache = shared_pricing_cache
        else:
            self._pricing_cache = {}
        if defer_pricing_warm:
            if not self._pricing_cache:
                QTimer.singleShot(0, self._warm_pricing_cache)
        elif self._embedded_in_dialog:
            QTimer.singleShot(0, self._warm_pricing_cache)
        else:
        ...
        self.glass.pricingOptionsChanged.connect(self._schedule_glass_material_preview)
        self.glass.clearHoldComplete.connect(self._on_hold_clear_all)
        ...
        self._sync_glass_visibility_from_client()
        if self._embedded_in_dialog:
            if self._facade_pick_mode:
                self._sync_facade_grid_rows()
    ...
    def showEvent(self, event):
        ...
        # Во вложенном режиме (вкладка в GlassMirrorCalcDialog / фасад) не фиксируем размер —
        # иначе на Windows мелькает отдельное «маленькое» окно и дёргается родительский диалог.
        if self.parent() is not None:
            return
        self._apply_compact_window_size()

    def _tile_row(self, logical_row: int) -> int:
        """Строка сетки плиток 1..4; во вложенном диалоге полоска клиента вынесена наверх."""
        if getattr(self, "_embedded_in_dialog", False):
            return max(0, int(logical_row) - 1)
        return int(logical_row)

    def _sync_facade_grid_rows(self) -> None:
        """В выборе стекла для фасада — без пустых строк сетки под скрытые плитки."""
        grid = self.layout()
        if grid is None or not getattr(self, "_facade_pick_mode", False):
            return
        sh = getattr(self, "_secondary_shown", False)
        grid.setRowMinimumHeight(0, 0)
        if sh:
            for r in (1, 2, 3):
                grid.setRowMinimumHeight(r, TILE_SIDE_PX)
        else:
            for r in (1, 2, 3, 4):
                grid.setRowMinimumHeight(r, 0)
        grid.setRowMinimumHeight(4, 0)

    def _apply_facade_pick_window_size(self) -> None:
        """Компактный размер встроенного калькулятора в диалоге фасада."""
        grid = self.layout()
        if grid is None:
            return
        self._sync_facade_grid_rows()
    ...
    def _sync_embedded_grid_row_heights(self) -> None:
        if not getattr(self, "_embedded_in_dialog", False):
            return
        grid = self.layout()
        if grid is None:
            return
        resolved = self._client_is_resolved()
        sh = getattr(self, "_secondary_shown", False)
        if not resolved:
            for r in (1, 2, 3, 4):
                grid.setRowMinimumHeight(self._tile_row(r), 0)
            return
        grid.setRowMinimumHeight(self._tile_row(1), TILE_SIDE_PX)
        grid.setRowMinimumHeight(self._tile_row(2), TILE_SIDE_PX)
        for r in (3, 4):
            grid.setRowMinimumHeight(self._tile_row(r), TILE_SIDE_PX if sh else 0)

    def _embedded_full_layout_size(self) -> tuple[int, int]:
        """Высота/ширина сетки как при полностью раскрытых плитках — окно не растёт по ходу работы."""
        cached = getattr(self, "_embedded_full_size_cache", None)
        if cached is not None:
            return cached
        grid = self.layout()
        if grid is None:
            return (400, 360)
        saved_rows = [grid.rowMinimumHeight(self._tile_row(r)) for r in (1, 2, 3, 4)]
        saved_glass = self.glass.isVisible()
        sh = getattr(self, "_secondary_shown", False)
        for r in (1, 2, 3, 4):
            grid.setRowMinimumHeight(self._tile_row(r), TILE_SIDE_PX)
        ...
        for i, r in enumerate((1, 2, 3, 4)):
            grid.setRowMinimumHeight(self._tile_row(r), saved_rows[i])
        self.glass.setVisible(saved_glass)
        for w in getattr(self, "_dependent_widgets", ()):
            if w is not None:
                w.setVisible(sh)
        self._sync_embedded_grid_row_heights()
        size = (max(m.width(), 400), max(m.height(), 360))
        self._embedded_full_size_cache = size
        return size
    ...
    def _apply_compact_window_size(self):
        ...
        if self.layout() is None:
            return
        if getattr(self, "_facade_pick_mode", False):
            self._apply_facade_pick_window_size()
            return
        if getattr(self, "_embedded_in_dialog", False):
            w, h = self._embedded_full_layout_size()
    ...
    def _on_glass_local_metrics(self):
        if hasattr(self, "_local_metrics_timer"):
            self._local_metrics_timer.start()
            return
        self._on_glass_local_metrics_impl()

    def _on_glass_local_metrics_impl(self):
        if not getattr(self, "_secondary_shown", False) and not self._client_is_resolved():
            return
        ...
        try:
            ...
            if conn is not None:
                try:
                    ...
                    pass
        if isinstance(self._pricing_cache, dict):
            self._pricing_cache.clear()
            self._pricing_cache.update(cache)
        else:
    ...
    def _on_client_identity_changed(self, *_args):
        """Смена клиента: сброс кэша фурнитуры и синхронизация видимости блоков."""
        self.furniture.set_pricing_cache(None, 1)
        self._sync_glass_visibility_from_client()
        if self._client_is_resolved() and getattr(self, "_secondary_shown", False):
            self._recalc_debounce.stop()
            self._run_recalculate_prices()
    ...
    def _client_is_resolved(self) -> bool:
        if getattr(self, "_facade_pick_mode", False):
            return True
        cs = self.client_strip
        if cs is None:
            return False
        return bool(getattr(cs, "_client_id", None)) or bool(getattr(cs, "_quick_client_id", None))

    def _sync_glass_visibility_from_client(self) -> None:
        if getattr(self, "_facade_pick_mode", False):
            self.glass.setVisible(True)
            return
        resolved = self._client_is_resolved()
        self.glass.setVisible(resolved)
        if not resolved:
    ...
    def _set_all_secondary_visible(self, vis: bool):
        ...
        if getattr(self, "_facade_pick_mode", False):
            self._sync_facade_grid_rows()
        elif getattr(self, "_embedded_in_dialog", False):
            self._sync_embedded_grid_row_heights()

    def _on_glass_configuration_dirty(self):
        """Изменение стекла: не скрываем доп. блоки — только снимаем «рассчитано» и обновляем превью цены."""
        self.glass.set_glass_border_highlight(False)
        self._schedule_glass_material_preview()
        if not getattr(self, "_embedded_in_dialog", False):
            self._apply_compact_window_size()
    ...
    def _require_client_resolved(self, action_label: str = "") -> bool:
        """Клиент из справочника обязателен до расчёта и добавления изделий."""
        if getattr(self, "_facade_pick_mode", False):
            return True
        ...
        if ok:
            return True
        if self.client_strip:
            self.client_strip._refresh_client_border_hint()
        hint = (
            "Сначала выберите клиента из справочника"
            + (" (перед «%s»)." % action_label if action_label else ".")
        )
        QMessageBox.warning(self, "Клиент", hint)
        return False

    def _on_glass_calculate_pressed(self):
        if not self._require_client_resolved("Рассчитать"):
            return
    ...
    def _on_glass_additional_pressed(self):
        ...
        """
        if not getattr(self, "_facade_pick_mode", False):
            if not self._require_client_resolved("Рассчитать"):
                return
        if not self.glass.is_ready_for_pricing():
            QMessageBox.warning(
    ...
    def _schedule_glass_material_preview(self, *_args):
        if hasattr(self, "_glass_preview_timer"):
            self._glass_preview_timer.start()

    def _refresh_glass_material_preview(self):
        """Цена материала с наценкой клиента до полного «Рассчитать» / при смене клиента."""
        if not self._client_is_resolved():
            self.glass.set_material_cost_label("Материал: выберите клиента")
            return
        if not self.glass.is_ready_for_pricing():
            self.glass.set_material_cost_label("Материал: укажите материал и размеры")
            return
        try:
            snap = copy.deepcopy(self.glass.build_selected(False))
            snap["Клиент"] = self.client_strip.get_payload() if self.client_strip else {}
            mc = compute_material_cost(snap)
            if not mc:
                self.glass.set_material_cost_label("Материал: укажите материал и размеры")
                return
            mkf = _client_markup_factor(snap.get("Клиент") or {})
            pm2 = _money_ceiled(mc.get("Цена за м² (₽)") or 0, mkf)
            s1 = _money_ceiled(mc.get("Стоимость за изделие (₽)") or 0, mkf)
            s_all = _money_ceiled(mc.get("Общая стоимость (₽)") or 0, mkf)
            izd = snap.get("Параметры изделия") or {}
            try:
                qn = int(izd.get("Количество (шт)") or 1)
            except (TypeError, ValueError):
                qn = 1
            if qn > 1:
                txt = "Материал: за м² %s ₽, за изд. %s ₽, всего %s ₽" % (pm2, s1, s_all)
            else:
                txt = "Материал: за м² %s ₽, изделие %s ₽" % (pm2, s1)
            if mkf > 1.0:
                txt += " (наценка клиента)"
    ...
    def _on_hold_clear_all(self):
        """Удержание «Очистить»: все блоки видны, значения — как при новом заказе."""
        self.selected = {}
        self.glass.reset_to_defaults()
        self._reset_secondary_widgets()
        self.furniture.set_pricing_cache(None, 1)
        self._recalc_debounce.stop()
        self._holes_recalc_timer.stop()
        sh = getattr(self, "_secondary_shown", False)
        self._set_all_secondary_visible(sh)
        self._refresh_glass_material_preview()
        self._update_footer_cost_labels()
        self._apply_compact_window_size()
    ...
    def initUI(self):
        grid = QGridLayout()
        grid.setSpacing(TILE_GRID_GAP)
        grid.setVerticalSpacing(TILE_GRID_GAP)
        grid.setHorizontalSpacing(TILE_GRID_GAP)
        grid.setContentsMargins(
            TILE_GRID_GAP,
            0 if getattr(self, "_embedded_in_dialog", False) else TILE_GRID_GAP,
            0,
            0 if getattr(self, "_embedded_in_dialog", False) else TILE_GRID_GAP,
        )
        for r in (1, 2, 3, 4):
            if getattr(self, "_embedded_in_dialog", False):
                ...
                grid.setRowMinimumHeight(self._tile_row(r), TILE_SIDE_PX)
            grid.setRowStretch(self._tile_row(r), 0)
        for c in range(6):
        ...
        self.setStyleSheet(
            "#xxx { border: none; background-color: %s; }" % P.MAIN_WINDOW_BG
        )

        if not self._embedded_in_dialog:
            grid.addWidget(self.client_strip, 0, 0, 1, 6)
        else:
            self.client_strip.hide()

        for c in range(6):
            grid.setColumnMinimumWidth(c, TILE_SIDE_PX)

        _r1 = self._tile_row(1)
        _r2 = self._tile_row(2)
        _r3 = self._tile_row(3)
        _r4 = self._tile_row(4)

        grid.addWidget(self.glass, _r1, 0, 2, 2, Qt.AlignTop | Qt.AlignLeft)
        _at = Qt.AlignTop | Qt.AlignLeft
        grid.addWidget(self.polirovka, _r1, 2, 1, 1, _at)
        grid.addWidget(self.shlifovka, _r1, 3, 1, 1, _at)
        grid.addWidget(self.facet, _r1, 4, 1, 1, _at)
        grid.addWidget(self.plenka, _r1, 5, 1, 1, _at)

        grid.addWidget(self.otverst, _r2, 2, 1, 1, _at)
        grid.addWidget(self.virez, _r2, 3, 1, 1, _at)
        grid.addWidget(self.peskostroy, _r2, 4, 1, 1, _at)
        grid.addWidget(self.photo, _r2, 5, 1, 1, _at)

        grid.addWidget(self.uf, _r3, 0, 1, 1, _at)
        grid.addWidget(self.angle, _r3, 1, 1, 1, _at)
        grid.addWidget(self.pod, _r3, 2, 1, 1, _at)
        grid.addWidget(self.pokraska, _r3, 3, 1, 1, _at)
        grid.addWidget(self.furniture, _r3, 4, 1, 1, _at)
        grid.addWidget(self.srochno, _r3, 5, 1, 1, _at)

        # Нижний ряд: замер|доставка|монтаж (одна широкая плитка), упаковка, доп., кнопки.
        grid.addWidget(self.zamer, _r4, 0, 1, 3, _at)
        grid.addWidget(self.packaging, _r4, 3, 1, 1, _at)
        grid.addWidget(self.dopi, _r4, 4, 1, 1, _at)
        ...
        _cost_bg = P.MAIN_WINDOW_BG
        _cost_lbl_css = (
            "font-size: 12px; color: %s; font-weight: 600; background-color: %s; padding: 0 2px;"
            % (P.TILE_TEXT, _cost_bg)
        )
        self._lbl_cost_one = QLabel("1 изделие: —")
        self._lbl_cost_one.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        self._lbl_cost_one.setWordWrap(True)
        self._lbl_cost_one.setStyleSheet(_cost_lbl_css)
        self._lbl_cost_order = QLabel("Заказ: —")
        self._lbl_cost_order.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        self._lbl_cost_order.setWordWrap(True)
        self._lbl_cost_order.setStyleSheet(_cost_lbl_css)
        self._lbl_cost_sum = QLabel("Сумма: —")
        self._lbl_cost_sum.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        self._lbl_cost_sum.setWordWrap(True)
        self._lbl_cost_sum.setStyleSheet(
            "font-size: 13px; color: #1b5e20; font-weight: bold; background-color: %s; padding: 0 2px;"
            % _cost_bg
        )
        ...
        self._actions_bar.setStyleSheet(
            "#mainActionsBar { background-color: %s; border: none; }"
            "#mainActionsBar QLabel { background-color: %s; }"
            % (P.MAIN_WINDOW_BG, P.MAIN_WINDOW_BG)
        )
        ...
        cost_box.setSpacing(2)
        for lb in (self._lbl_cost_one, self._lbl_cost_order, self._lbl_cost_sum):
            cost_box.addWidget(lb, 0, Qt.AlignLeft)
        if not self._embedded_in_dialog:
            actions_lay.addLayout(cost_box, 0)
        else:
            for lb in (self._lbl_cost_one, self._lbl_cost_order, self._lbl_cost_sum):
                lb.hide()
        ...
        for b in _btn_row:
            btn_box.addWidget(b, 0, Qt.AlignHCenter)
        actions_lay.addLayout(btn_box, 0)

        self._actions_bar.setMinimumWidth(int(TILE_SIDE_PX))
        self._actions_bar.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Expanding)
        _bar_align = Qt.AlignBottom | Qt.AlignRight if self._embedded_in_dialog else Qt.AlignTop | Qt.AlignRight
        grid.addWidget(self._actions_bar, _r4, 5, 1, 1, _bar_align)
    ...
    def _export_order_pdf(self):
        ...
        from logic.pdf_storage import resolve_pdf_path_for_blocks_export
        ...
        try:
            ...
            return
        cli = self.client_strip.get_payload() if self.client_strip else {}
        cname = (cli.get("Имя") or "").strip() if isinstance(cli, dict) else ""
        path = resolve_pdf_path_for_blocks_export(cname, is_estimate=True)
        try:
    ...
    def _apply_selected_delivery_block_from_zamer(self, *, conn=None):
        ...
        base_rub = delivery_base_rub_from_data(dd, del_tab)
        pr = delivery_price_rub(del_tab, inside, km, base_rub=base_rub)
        dd["Доставка цена"] = pr
        self.selected["Доставка"] = {"Активирован": True, "Данные": dd}
    ...
    def _recalculate_impl(self, upload_sketch: bool = False):
        ...
        try:
            ...
            if self.packaging.is_any_selected():
                tier = None
                try:
                    cli = self.selected.get("Клиент") or {}
                    cid = cli.get("id")
                    if cid:
                        from db import models as db_models

                        crow = db_models.get_client_by_id(int(cid)) or {}
                        tier = crow.get("pricing_tier")
                except Exception:
                    tier = None
                pb = compute_packaging_block(izd, self.packaging.flags(), plow, pricing_tier=tier)
                if pb.get("Ошибка"):
            ...
            if zblk.get("Активирован") and zd:
                if (bool(zd.get("Замер")) or bool(zd.get("Монтаж")) or (not bool(zd.get("Без замера")))) and self.zamer.is_configured_for_visit_price():
                    from calc.delivery_calc import montazh_price_rub, zamer_visit_price_rub

                    del_tab = self._pricing_cache.get("delivery_prices") or fetch_delivery_prices(conn=conn)
                    vd = zd.get("Данные выезда") or {}
                    inside = bool(vd.get("Внутри КАД", True))
                    km = vd.get("Расстояние до КАД")
                    has_delivery = bool(zd.get("Доставка"))
                    has_measure = bool(zd.get("Замер"))
                    has_install = bool(zd.get("Монтаж"))
                    base_rub = self.zamer.get_delivery_base_rub()
                    if has_delivery or (has_install and not has_measure):
                        pr = delivery_price_rub(del_tab, inside, km, base_rub=base_rub)
                    elif has_measure and not has_delivery and not has_install:
                        pr = int(zamer_visit_price_rub(del_tab, inside, km) or 0)
                    else:
                        pr = delivery_price_rub(del_tab, inside, km, base_rub=base_rub)
                        if has_install:
                            pr += int(montazh_price_rub(del_tab) or 0)
                    zd["Замер цена выезда"] = pr
                    lines = []
                    if inside:
                        if has_delivery or (has_install and not has_measure):
                            lines.append("Базовый тариф доставки: %s ₽" % int(base_rub))
                        else:
                            pin = int(del_tab.get("Замер", del_tab.get("В пределах КАД", 0)) or 0)
                            lines.append("Тариф выезда в пределах КАД: %s ₽" % pin)
                        lines.append("Итого выезд: %s ₽" % pr)
                    else:
                        base = int(base_rub if has_delivery or has_install else del_tab.get("За КАД база", 0) or 0)
    ...
    def _update_footer_cost_labels(self):
        if not hasattr(self, "_lbl_cost_sum"):
            return
        try:
            from calc.order_summary import collect_line_items

            rows, total, _warns = collect_line_items(self.selected, self, conn=None)
            q_izd = int((self.selected.get("Параметры изделия") or {}).get("Количество (шт)") or 1)
            per_one = int(round(int(total) / q_izd)) if q_izd > 0 else int(total)
            self._lbl_cost_one.setText("1 изделие: %s ₽" % per_one)
            if q_izd > 1:
                self._lbl_cost_order.setText("Заказ (%s шт.): %s ₽" % (q_izd, int(total)))
            else:
                self._lbl_cost_order.setText("Заказ: %s ₽" % int(total))
            self._lbl_cost_sum.setText("Сумма: %s ₽" % int(total))
            if self._embedded_in_dialog:
                self.calcSummaryChanged.emit()
        except Exception:
            self._lbl_cost_one.setText("1 изделие: —")
            self._lbl_cost_order.setText("Заказ: —")
            self._lbl_cost_sum.setText("Сумма: —")
