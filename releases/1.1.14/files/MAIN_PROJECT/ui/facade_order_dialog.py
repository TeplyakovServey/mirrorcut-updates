...
from calc.delivery_calc import delivery_price_rub, fetch_delivery_prices, delivery_base_rub_from_data
...
class FacadeOrderDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        install_client_search_layout_helper(self.client_edit)
        ...
        self._facade_tabs_bar.setTabsClosable(True)
        self._facade_tabs_bar.tabCloseRequested.connect(self._on_facade_tab_close_requested)
        ...
        self.rb_screw_black = QRadioButton("черный")
        ...
        self.rb_screw_black.toggled.connect(self._on_screw_variant_changed)
        screw_radio_row.addWidget(self.rb_screw_silver)
        screw_radio_row.addWidget(self.rb_screw_gold)
        screw_radio_row.addWidget(self.rb_screw_black)
        ...
        self._btn_add_facade.setAutoDefault(False)
        self._btn_add_facade.setDefault(False)
        ...
        self._btn_maket = QPushButton("Макет")
    ...
    def _calc_service_prices(self):
        ...
        if dblk.get("Активирован") and isinstance(dblk.get("Данные"), dict):
            ...
            if inside or km is not None:
                base = delivery_base_rub_from_data(dd, dp)
                delivery_total = int(delivery_price_rub(dp, inside, km, base_rub=base) or 0)
        zamer_visit_total = 0
        ...
        if zblk.get("Активирован") and isinstance(zd, dict):
            ...
            if need_visit and (inside or km is not None):
                base = delivery_base_rub_from_data(vd, dp)
                if bool(zd.get("Доставка")):
                    zamer_visit_total = int(delivery_price_rub(dp, inside, km, base_rub=base) or 0)
                elif bool(zd.get("Замер")) and not bool(zd.get("Монтаж")):
                    from calc.delivery_calc import zamer_visit_price_rub
                    zamer_visit_total = int(zamer_visit_price_rub(dp, inside, km) or 0)
                else:
                    zamer_visit_total = int(delivery_price_rub(dp, inside, km, base_rub=base) or 0)
    ...
    def _apply_tab_state_to_ui(self, st: dict[str, Any]) -> None:
        ...
        try:
            ...
            self._sync_screw_radio_buttons()
    ...
    def _refresh_facade_tabs_bar(self) -> None:
        ...
        if n >= 2:
            ...
            self._facade_tabs_bar.tabBar().setTabButton(0, QTabBar.RightSide, None)
        self._facade_tabs_bar.blockSignals(False)

    def _on_facade_tab_close_requested(self, index: int) -> None:
        if index <= 0 or len(self._facade_tabs) <= 1:
            return
        ...
        del self._facade_tabs[index]
        if self._active_facade_tab_idx >= len(self._facade_tabs):
            self._active_facade_tab_idx = len(self._facade_tabs) - 1
        elif self._active_facade_tab_idx > index:
            self._active_facade_tab_idx -= 1
        self._refresh_facade_tabs_bar()
        if self._facade_tabs:
            self._apply_tab_state_to_ui(self._facade_tabs[self._active_facade_tab_idx])

    def keyPressEvent(self, event):
        if event.key() in (Qt.Key_Delete, Qt.Key_Period):
            w = QApplication.focusWidget()
            if w is not None and isinstance(w, (QLineEdit, QSpinBox, QComboBox, QTextEdit, QPlainTextEdit)):
    ...
    def _on_add_facade_tab(self) -> None:
        if not self._require_facade_client("Добавить фасад"):
            return
    ...
    def _get_screw_prices(self) -> dict[str, float]:
        ...
        out: dict[str, float] = {"серебро": 0.0, "золото": 0.0, "черный": 0.0}
        rows = facades_get_all_angle_seal()
        for r in rows or []:
            ...
            if "сереб" in v:
                ...
                out["черный"] = float(r.get("price") or 0)
        if out["черный"] <= 0 and out["серебро"] > 0:
            out["черный"] = out["серебро"]
        self._screw_price_cache = out
        return out

    def _screw_icon_path(self, color_variant: str) -> str | None:
        cv = str(color_variant or "").lower()
        if "золот" in cv:
            key = "золото"
        elif "черн" in cv:
            key = "черный"
        ...
        target = "gold.png" if key == "золото" else ("black.png" if key == "черный" else "silver.png")
        try:
    ...
    def _on_master_hole_locks_toggled(self, checked):
        if not self._fitting_selected_side:
            return
        if getattr(self, "_master_lock_busy", False):
            return
        self._master_lock_busy = True
        try:
            self.facade_canvas.set_all_hole_locks(bool(checked))
        finally:
            self._master_lock_busy = False
    ...
    def _require_facade_client(self, action_label: str = "") -> bool:
        from db import models as db_models
        ...
        if not cid and not qcid and cn:
            try:
                ...
                cid = None
            if not cid and getattr(self, "_quick_estimate_mode", False):
                try:
                    qid = db_models.get_mirror_quick_client_id_by_name(cn)
                    qcid = int(qid) if qid is not None else None
                except Exception:
                    qcid = None
        if cid or qcid:
            return True
    ...
    def _sync_screw_radio_buttons(self) -> None:
        v = str(self._screw_variant or "серебро").lower()
        if getattr(self, "rb_screw_silver", None):
            self.rb_screw_silver.setChecked(v == "серебро")
        if getattr(self, "rb_screw_gold", None):
            self.rb_screw_gold.setChecked(v == "золото")
        if getattr(self, "rb_screw_black", None):
            self.rb_screw_black.setChecked(v == "черный")
    ...
    def _on_screw_variant_changed(self, _checked: bool = True) -> None:
        if getattr(self, "rb_screw_gold", None) and self.rb_screw_gold.isChecked():
            self._screw_variant = "золото"
        elif getattr(self, "rb_screw_black", None) and self.rb_screw_black.isChecked():
            self._screw_variant = "черный"
        else:
            self._screw_variant = "серебро"
        self._update_materials_summary_panel()
    ...
    def _facade_restore_glass_dims(self, app):
        """После «Очистить» в фасаде — снова подставить размеры из проёма (поля заблокированы)."""
        ...
        gx._w_rect.setText(str(w))
        gx._h_rect.setText(str(h))
        gx.lbl_mat_cost.setText(
    ...
    def _open_glass_pick_dialog(self):
        ...
        if self._glass_main_app is None:
            ...
            g0 = app.glass
            matp0 = initial_payload.get("Параметры материала") or {}
            if not str(matp0.get("Тип материала") or "").strip():
                g0.select_preferred_default_material()
            ...
            if getattr(g0, "clearHoldComplete", None) is not None:
                g0.clearHoldComplete.connect(lambda _a=app: self._facade_restore_glass_dims(_a))
            ...
            btn_finish.setMinimumHeight(36)
            btn_finish.setMaximumHeight(36)
            ...
            app._sync_facade_grid_rows()
            app._apply_facade_pick_window_size()
        else:
        ...
        if getattr(app, "_facade_pick_mode", False):
            app._sync_facade_grid_rows()
            app._apply_facade_pick_window_size()
        ...
        class _GlassPickDlgSizeSync(QObject):
            ...
            def _fit(self):
                ...
                if getattr(app, "_facade_pick_mode", False):
                    app._apply_facade_pick_window_size()
                self._lay.activate()
                hint = self._lay.minimumSize()
                dlg.resize(max(hint.width(), 400), max(hint.height(), 280))
    ...
    def _on_glass_clicked(self):
        if not self._require_facade_client("Выбор стекла"):
            return
        if not self._side_profiles:
            QMessageBox.information(
                self,
