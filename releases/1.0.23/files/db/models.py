...
# --- Full sheets ---
FULL_SHEET_SELECT = (
    "id, name, height_mm, width_mm, thickness_mm, arrival_date, supplier_id, supplier, "
        ...
        with conn.cursor() as cur:
            cur.execute(
                """INSERT INTO mirror_full_sheets (
                    name, height_mm, width_mm, thickness_mm, arrival_date, supplier_id, supplier,
                    cost, warehouse_number, quantity, comment
        ...
        with conn.cursor() as cur:
            cur.execute(
                """UPDATE mirror_suppliers SET
                    ...
                    first_name=%s, last_name=%s, passport_series=%s, passport_number=%s, birth_date=%s, gender=%s,
                    phone=%s, email=%s, legal_address=%s, actual_address=%s, source=%s, notes=%s WHERE id=%s""",
                (
...
def get_supplier_deliveries(supplier_id, limit=25):
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """SELECT id, name, height_mm, width_mm, thickness_mm, arrival_date, supplier, supplier_id,
                          cost, warehouse_number, quantity, comment
