...
_ORDER_PORTAL_FILTER_STATES = ("none", "pending", "progress", "done")
_ORDER_PORTAL_FILTER_STATE_RU = {
    "none": "не нужна",
    "pending": "ожидание",
    "progress": "в работе",
    "done": "готово",
...
def fetch_orders_list_snapshot(force_refresh: bool = False) -> dict:
    """Сбор данных главной таблицы заказов (без Qt; можно из фонового потока)."""
    from db import models as db_models

    orders = db_models.get_orders_all(force_refresh=bool(force_refresh))
    ...
    order_cut_ids = frozenset()
    blocks_zamer_by_order = {}
    orders_items = {}
    production_events_by_order = {}
    if oids:
        try:
            order_cut_ids = frozenset(db_models.get_order_ids_with_cut_results(oids))
        except Exception:
            order_cut_ids = frozenset()
        try:
            blocks_zamer_by_order = db_models.get_blocks_zamer_rows_by_mirror_order_ids(oids) or {}
        except Exception:
            blocks_zamer_by_order = {}
        try:
            bulk = db_models.get_order_items_bulk(oids) or {}
            for o in orders:
                oid = o.get("id")
                if oid:
                    orders_items[oid] = list(bulk.get(oid) or [])
        except Exception:
            for o in orders:
                oid = o.get("id")
                if not oid:
                    continue
                try:
                    orders_items[oid] = db_models.get_order_items(oid) or []
                except Exception:
                    orders_items[oid] = []
        try:
            production_events_by_order = db_models.list_production_events_for_orders(oids) or {}
        except Exception:
            production_events_by_order = {}
    return {
        ...
        "order_cut_ids": order_cut_ids,
        "blocks_zamer_by_order": dict(blocks_zamer_by_order or {}),
        "orders_items": dict(orders_items or {}),
        "production_events_by_order": dict(production_events_by_order or {}),
    }


class _OrdersListFetchThread(QThread):
    """Полная перезагрузка списка заказов в фоне — UI не блокируется."""
    ...
    def run(self):
        try:
            snap = fetch_orders_list_snapshot(self._force_refresh)
        except Exception:
            snap = None
...
class _OrderSurchargeDelegate(_OrderMetaCellDelegate):
    def _surcharge_display_text(self, m: dict, fallback: str) -> str:
        total = int(m.get("surcharge_total") or 0)
        paid_raw = int(m.get("surcharge_paid") or 0)
        if total <= 0 and paid_raw <= 0:
            return fallback if fallback and fallback != "—" else "—"
        methods = ""
        sur_lc = (m.get("surcharge_lc") or "").strip()
        if sur_lc and "(" in sur_lc:
            methods = sur_lc.split("(", 1)[-1].rstrip(")")
        txt = MainWindow._format_surcharge_short(paid_raw, total, methods)
        if txt and txt != "—":
            return txt
        return fallback if fallback and fallback != "—" else "—"
    ...
    def paint(self, painter, option, index):  # noqa: N802
        ...
        m = self._meta(index)
        o = self._win._order_for_table_row(index.row()) or {}
        fallback = str(index.data(Qt.DisplayRole) or "")
        total = int(m.get("surcharge_total") or 0)
        paid_raw = int(m.get("surcharge_paid") or 0)
        paid = min(total, max(0, paid_raw)) if total > 0 else max(0, paid_raw)
        text = self._surcharge_display_text(m, fallback)
        no_surcharge = self._win._is_sales_row(o) or (total <= 0 and paid_raw <= 0)
        if self._paint_selection(painter, option, index):
            painter.setPen(QColor(_ORDER_ROW_SEL_FG))
            ...
            denom = total if total > 0 else max(1, paid_raw)
            ratio = paid / float(denom)
...
class MainWindow(QWidget):
    def __init__(self, user, parent=None, startup_splash=None):
        ...
        self._orders_layout_resync_timer = QTimer(self)
        self._orders_layout_resync_timer.setSingleShot(True)
        self._orders_layout_resync_timer.setInterval(50)
        self._orders_layout_resync_timer.timeout.connect(self._apply_orders_table_layout_resync)
        self._orders_table_pending_column_sizes = None
        self._orders_layout_resync_running = False
        self._orders_layout_resync_pending = False
        self._orders_notify_pending = set()
        self._orders_notify_timer = QTimer(self)
        self._orders_notify_timer.setSingleShot(True)
        try:
            self._orders_notify_ms = int((os.environ.get("MC_ORDERS_LISTEN_UI_MS") or "80").strip())
        except ValueError:
            self._orders_notify_ms = 80
        self._orders_notify_ms = max(40, min(500, self._orders_notify_ms))
        self._orders_notify_timer.setInterval(self._orders_notify_ms)
        self._orders_notify_timer.timeout.connect(self._flush_orders_list_notify_batch)
        self._orders_list_fetch_thread = None
        self._orders_list_fetch_pending = False
        self._orders_list_listen = None
    ...
    def _build_ui(self):
        ...
        btn_delivery_est = QPushButton("Просчет доставки")
        btn_delivery_est.setStyleSheet(
            "QPushButton { background-color: #3d8f72; color: white; padding: 10px 20px; "
            "font-weight: bold; border: none; border-radius: 6px; }"
            "QPushButton:hover { background-color: #47a383; }"
            "QPushButton:pressed { background-color: #2f7560; }"
        )
        btn_delivery_est.clicked.connect(self._on_delivery_estimate)
        header_layout.addWidget(btn_delivery_est)
        ...
        self._orders_panel.installEventFilter(self)
        ...
        self._quick_filter_reset = QPushButton("Сброс фильтров")
    ...
    def _build_orders_filter_bar(self):
        ...
        gs.setSpacing(6)
        _lbl_g = QLabel("Поиск клиента по тел/ИНН:")
        ...
        self._install_cyrillic_layout_helper(self._flt_client_phone)
        ...
        self._install_cyrillic_layout_helper(self._flt_col_client)
        ...
        self._install_cyrillic_layout_helper(self._flt_col_manager)
        ...
        _ls.setSpacing(2)
        self._flt_surcharge_from = QLineEdit()
        self._flt_surcharge_from.setPlaceholderText("₽ от")
        self._flt_surcharge_from.setStyleSheet(le_st)
        self._flt_surcharge_from.setToolTip("Сумма доплаты по заказу — от")
        self._flt_surcharge_from.textChanged.connect(self._schedule_apply_order_filters)
        self._flt_surcharge_to = QLineEdit()
        self._flt_surcharge_to.setPlaceholderText("₽ до")
        self._flt_surcharge_to.setStyleSheet(le_st)
        self._flt_surcharge_to.setToolTip("Сумма доплаты по заказу — до")
        self._flt_surcharge_to.textChanged.connect(self._schedule_apply_order_filters)
        _ls.addWidget(self._flt_surcharge_from)
        _ls.addWidget(self._flt_surcharge_to)
        _c5s = wrap_cell(_ws)
        ...
        self._flt_meas_partial_combo = QComboBox()
        self._flt_meas_partial_combo.setStyleSheet(cb_st)
        self._flt_meas_full_combo = QComboBox()
        self._flt_meas_full_combo.setStyleSheet(cb_st)
        self._flt_meas_partial_combo.setToolTip(
            "Замер: хотя бы один статус услуги в заказе совпадает с выбранным."
        )
        self._flt_meas_full_combo.setToolTip(
            "Замер: статус услуги в заказе совпадает с выбранным."
        )
        _wm = QWidget()
        _lm = QVBoxLayout(_wm)
        _lm.setContentsMargins(0, 0, 0, 0)
        _lm.setSpacing(2)
        _lm.addWidget(self._flt_meas_partial_combo)
        _lm.addWidget(self._flt_meas_full_combo)
        _c7 = wrap_cell(_wm)
        self._order_filter_cells.append(_c7)
        fb.addWidget(_c7, 0, Qt.AlignTop)

        self._flt_del_partial_combo = QComboBox()
        self._flt_del_partial_combo.setStyleSheet(cb_st)
        self._flt_del_full_combo = QComboBox()
        self._flt_del_full_combo.setStyleSheet(cb_st)
        self._flt_del_partial_combo.setToolTip(
            "Доставка: хотя бы один статус услуги в заказе совпадает с выбранным."
        )
        self._flt_del_full_combo.setToolTip(
            "Доставка: статус услуги в заказе совпадает с выбранным."
        )
        _wd = QWidget()
        _ld = QVBoxLayout(_wd)
        _ld.setContentsMargins(0, 0, 0, 0)
        _ld.setSpacing(2)
        _ld.addWidget(self._flt_del_partial_combo)
        _ld.addWidget(self._flt_del_full_combo)
        _c8 = wrap_cell(_wd)
        self._order_filter_cells.append(_c8)
        fb.addWidget(_c8, 0, Qt.AlignTop)

        self._flt_inst_partial_combo = QComboBox()
        self._flt_inst_partial_combo.setStyleSheet(cb_st)
        self._flt_inst_full_combo = QComboBox()
        self._flt_inst_full_combo.setStyleSheet(cb_st)
        self._flt_inst_partial_combo.setToolTip(
            "Монтаж: хотя бы один статус услуги в заказе совпадает с выбранным."
        )
        self._flt_inst_full_combo.setToolTip(
            "Монтаж: статус услуги в заказе совпадает с выбранным."
        )
        _wi = QWidget()
        _li = QVBoxLayout(_wi)
        _li.setContentsMargins(0, 0, 0, 0)
        _li.setSpacing(2)
        _li.addWidget(self._flt_inst_partial_combo)
        _li.addWidget(self._flt_inst_full_combo)
        _c9 = wrap_cell(_wi)
        self._order_filter_cells.append(_c9)
        fb.addWidget(_c9, 0, Qt.AlignTop)

        self._populate_portal_status_filter_combos()
        self._flt_meas_partial_combo.currentIndexChanged.connect(self._schedule_apply_order_filters)
        self._flt_meas_full_combo.currentIndexChanged.connect(self._schedule_apply_order_filters)
        self._flt_del_partial_combo.currentIndexChanged.connect(self._schedule_apply_order_filters)
        self._flt_del_full_combo.currentIndexChanged.connect(self._schedule_apply_order_filters)
        self._flt_inst_partial_combo.currentIndexChanged.connect(self._schedule_apply_order_filters)
        self._flt_inst_full_combo.currentIndexChanged.connect(self._schedule_apply_order_filters)
        ...
        _l11.setSpacing(4)
        _bclr = QPushButton("Сброс фильтров")
        _bclr.setStyleSheet(
        ...
        _bupd = QPushButton("Обновить базу")
        _bupd.setToolTip("Перезагрузить всю таблицу заказов из базы данных.")
        _bupd.setStyleSheet(
            "QPushButton { font-size:10px; padding:3px 6px; border-radius:3px; border:1px solid #64b5f6; background:%s; color:#1a365d; }"
    ...
    def _populate_portal_status_filter_combos(self):
        portal_combos = (
            getattr(self, "_flt_meas_partial_combo", None),
            getattr(self, "_flt_meas_full_combo", None),
            getattr(self, "_flt_del_partial_combo", None),
            getattr(self, "_flt_del_full_combo", None),
            getattr(self, "_flt_inst_partial_combo", None),
            getattr(self, "_flt_inst_full_combo", None),
        )
        prefixes = ("≥1: ", "все: ", "≥1: ", "все: ", "≥1: ", "все: ")
        for combo, prefix in zip(portal_combos, prefixes):
            if combo is None:
                continue
            combo.blockSignals(True)
            combo.clear()
            combo.addItem("%s—" % prefix, "")
            for key in _ORDER_PORTAL_FILTER_STATES:
                combo.addItem(
                    "%s%s" % (prefix, _ORDER_PORTAL_FILTER_STATE_RU.get(key, key)),
                    key,
                )
            combo.setCurrentIndex(0)
            combo.blockSignals(False)
    ...
    def eventFilter(self, obj, event):  # noqa: N802
        watched = (
            getattr(self, "_orders_table", None),
            getattr(self, "_orders_panel", None),
        )
        if obj in watched and event.type() == QEvent.Resize:
            self._schedule_orders_table_layout_resync()
        return False
    ...
    def _on_orders_header_section_resized(self, logical_index, old_size, new_size):
        if getattr(self, "_orders_table_fit_in_progress", False):
            self._reflow_orders_table_rows_after_column_resize()
    ...
    def _orders_table_viewport_width(self):
        ...
        if tbl is None:
            return 1
        self._ensure_orders_table_geometry_ready()
        return max(1, int(tbl.viewport().width() or 0))

    def _ensure_orders_table_geometry_ready(self):
        """Дождаться актуальной геометрии viewport после resize окна/панели."""
        panel = getattr(self, "_orders_panel", None)
    ...
    def _apply_orders_table_column_sizes(self, sizes):
        ...
        if getattr(self, "_orders_table_fit_in_progress", False):
            self._orders_table_pending_column_sizes = sizes
            return
        tbl = getattr(self, "_orders_table", None)
        if tbl is None:
            return
        ...
        try:
            ...
            self._orders_table_fit_in_progress = False
        self._reflow_orders_table_rows_after_column_resize()
        self._sync_order_filter_bar_widths_apply()
        self._schedule_sync_filter_bar_widths()
        pending = getattr(self, "_orders_table_pending_column_sizes", None)
        self._orders_table_pending_column_sizes = None
        if pending is not None and list(pending) != sizes:
            QTimer.singleShot(0, lambda p=list(pending): self._apply_orders_table_column_sizes(p))

    def _orders_table_cell_wrap_text(self, tbl, row, col):
        it = tbl.item(row, col)
        if it is None:
            return ""
        text = (it.text() or "").strip()
        if not text or text == "—":
            text = (it.toolTip() or "").strip()
        return text

    def _reflow_orders_table_rows_after_column_resize(self):
        """Пересчитать высоту строк после смены ширины колонок (как при ручном drag заголовка)."""
        tbl = getattr(self, "_orders_table", None)
        ...
        default_h = max(int(vh.defaultSectionSize() or 38), int(vh.minimumSectionSize() or 22))
        fm = tbl.fontMetrics()
            ...
            for row in range(tbl.rowCount()):
                ...
                for col in wrap_cols:
                    if col >= tbl.columnCount():
                        continue
                    text = self._orders_table_cell_wrap_text(tbl, row, col)
                    if not text:
                        continue
                    cw = max(20, int(tbl.columnWidth(col) or 20) - 8)
                    bounds = fm.boundingRect(0, 0, cw, 0, Qt.AlignLeft | Qt.TextWordWrap, text)
                    max_h = max(max_h, int(bounds.height()) + 10)
                tbl.setRowHeight(row, max_h)
        finally:
            tbl.setUpdatesEnabled(True)
        try:
            tbl.scheduleDelayedItemsLayout()
        except Exception:
            pass
        tbl.viewport().update()
    ...
    def _schedule_orders_table_layout_resync(self):
        """Дебаунс: при ресайзе окна/панели пересчитать колонки и выровнять фильтры."""
        if getattr(self, "_main_table_mode", "") != "orders":
            return
        if getattr(self, "_orders_table", None) is None:
            return
        self._last_synced_section_widths = None
        if getattr(self, "_orders_layout_resync_timer", None) is not None:
            self._orders_layout_resync_timer.start()

    def _apply_orders_table_layout_resync(self):
        """Пропорции колонок + немедленное и отложенное выравнивание строки фильтров."""
        if getattr(self, "_main_table_mode", "") != "orders":
            return
        if getattr(self, "_orders_table", None) is None:
            return
        if getattr(self, "_orders_layout_resync_running", False):
            self._orders_layout_resync_pending = True
            return
        self._orders_layout_resync_running = True
        try:
            self._last_synced_section_widths = None
            self._ensure_orders_table_geometry_ready()
            use_def = not getattr(self, "_orders_table_columns_customized", False)
            self._apply_orders_table_proportional_layout(use_defaults=use_def)
            self._reflow_orders_table_rows_after_column_resize()
            self._sync_order_filter_bar_widths_apply()
            QTimer.singleShot(0, self._sync_order_filter_bar_widths_apply)
            QTimer.singleShot(50, self._sync_order_filter_bar_widths_apply)
            QTimer.singleShot(150, self._sync_order_filter_bar_widths_apply)
            QTimer.singleShot(300, self._sync_order_filter_bar_widths_apply)
        finally:
            self._orders_layout_resync_running = False
            if getattr(self, "_orders_layout_resync_pending", False):
                self._orders_layout_resync_pending = False
                QTimer.singleShot(0, self._apply_orders_table_layout_resync)
    ...
    def _schedule_sync_filter_bar_widths_chain(self):
        """Повторная подгонка после layout — фильтры сразу под колонками при старте."""
        self._last_synced_section_widths = None
        self._schedule_sync_filter_bar_widths()
        QTimer.singleShot(0, self._schedule_sync_filter_bar_widths)
        QTimer.singleShot(50, self._schedule_sync_filter_bar_widths)
        QTimer.singleShot(150, self._schedule_sync_filter_bar_widths)
    ...
    def _sync_order_filter_bar_widths_apply(self):
        ...
        if vh_w > 0 and sw == getattr(self, "_last_synced_section_widths", None):
            return
        if vh_w > 0:
    ...
    def _footer_orders_legend(self):
        ...
        def _legend_sep():
            ...
            line.setStyleSheet("background-color: #000000; border: none; max-width: 1px;")
            return line

        h.addSpacing(6)
        h.addWidget(_legend_sep(), 0)
        h.addSpacing(6)
    ...
    def _install_cyrillic_layout_helper(self, edit: QLineEdit) -> None:
    ...
    @staticmethod
    def _format_surcharge_short(paid: int, total: int, methods_txt: str = "") -> str:
        ...
        if paid >= total:
            short_m = ""
            if methods_txt:
                short_m = methods_txt.split(",")[0].strip()
                if "—" in short_m:
                    short_m = short_m.split("—", 1)[-1].strip()
            return "%s ₽ · %s" % (total, short_m) if short_m else "%s ₽" % total
        return "%s / %s ₽" % (paid, total)

    def _client_name_matches_filter(self, text: str, m: dict, o: dict) -> bool:
        if not text:
            return True
        try:
        ...
        variants = client_search_prefix_variants(text) or (text.strip().lower(),)
        for t in variants:
            if self._client_name_matches_filter_one(t, m, o):
                return True
        return False

    def _client_name_matches_filter_one(self, text: str, m: dict, o: dict) -> bool:
        if not text:
            return True
        hay = (
            m.get("client_search_haystack")
            or m.get("client_lc")
            or str(o.get("client_name") or "").lower()
        )
        if text in hay:
            return True
        tokens = [t for t in hay.split() if t]
        prefs = [t for t in text.split() if t]
        if not prefs:
            return True
        for pref in prefs:
            if any(t.startswith(pref) for t in tokens):
                continue
            if pref not in hay:
                return False
        return True
    ...
    def _on_async_filter_cache_ready(self, cache, meta_heavy, generation):
        ...
        try:
            for row, o in enumerate(disp):
                ...
                self._refresh_order_meta_dependent_cells(tbl, row, o)
        finally:
            tbl.setUpdatesEnabled(True)
        tbl.viewport().update()

    def _refresh_order_meta_dependent_cells(self, tbl, row, o):
        """Обновить ячейки, зависящие от heavy-meta (оплата, доплата, статус)."""
    ...
    def _order_portal_service_state(self, o, service: str) -> str:
        from ui.order_tile import order_service_cell_payload
        ...
        try:
            oid = int(o.get("id"))
            pg = (getattr(self, "_blocks_zamer_by_order", None) or {}).get(oid)
        except (TypeError, ValueError):
            pg = None
        try:
            p = order_service_cell_payload(o, pr, service, pg)
            return str(p.get("state") or "pending").strip() or "pending"
        except Exception:
            return "pending"

    def _order_matches_portal_partial_filter(self, o, service: str, state_key: str) -> bool:
        if not state_key:
            return True
        return self._order_portal_service_state(o, service) == state_key

    def _order_matches_portal_full_filter(self, o, service: str, state_key: str) -> bool:
        if not state_key:
            return True
        return self._order_portal_service_state(o, service) == state_key
    ...
    def _row_passes_active_filters(self, o):
        ...
        if qid and qid not in m.get("id_str", str(o.get("id") or "").lower()):
            return False
        text = normalize_cyrillic_search(self._flt_col_client.text() or "").strip().lower()
        if text and not self._client_name_matches_filter(text, m, o):
            return False
        ...
        if mgr:
            try:
                ...
                from MAIN_PROJECT.logic.keyboard_layout import client_search_prefix_variants
            mgr_variants = client_search_prefix_variants(mgr) or (mgr,)
            tokens = []
            for mv in mgr_variants:
                tokens.extend(t for t in mv.split() if t)
            if not tokens:
                tokens = [mgr]
            fn = (o.get("creator_name") or "").strip().lower()
            sn = (o.get("creator_surname") or "").strip().lower()
        ...
        if pay_key:
            if not self._order_matches_payment_filter_key(o, m, pay_key):
                return False
        ssf = self._parse_int_opt(
            (getattr(self, "_flt_surcharge_from", None) or QLineEdit()).text()
        )
        sst = self._parse_int_opt(
            (getattr(self, "_flt_surcharge_to", None) or QLineEdit()).text()
        )
        if ssf is not None or sst is not None:
            sur_total = int(m.get("surcharge_total") or 0)
            if sur_total <= 0:
                return False
            if ssf is not None and sur_total < int(ssf):
                return False
            if sst is not None and sur_total > int(sst):
                return False
        part_st = ""
        full_st = ""
        ...
        if ci == 2 and m.get("cut_cell") != "нет":
            return False
        portal_filters = (
            ("measure", getattr(self, "_flt_meas_partial_combo", None), getattr(self, "_flt_meas_full_combo", None)),
            ("delivery", getattr(self, "_flt_del_partial_combo", None), getattr(self, "_flt_del_full_combo", None)),
            ("install", getattr(self, "_flt_inst_partial_combo", None), getattr(self, "_flt_inst_full_combo", None)),
        )
        for svc, partial_cb, full_cb in portal_filters:
            part_st = ""
            full_st = ""
            if partial_cb is not None:
                part_st = str(partial_cb.currentData() or "").strip()
            if full_cb is not None:
                full_st = str(full_cb.currentData() or "").strip()
            if part_st and not self._order_matches_portal_partial_filter(o, svc, part_st):
                return False
            if full_st and not self._order_matches_portal_full_filter(o, svc, full_st):
                return False
        return True
    ...
    def _on_reset_all_order_filters(self):
        self._flt_col_surcharge_from = getattr(self, "_flt_surcharge_from", None)
        self._flt_col_surcharge_to = getattr(self, "_flt_surcharge_to", None)
        for w in (
            self._flt_col_id,
            self._flt_col_client,
            self._flt_col_manager,
            self._flt_surcharge_from,
            self._flt_surcharge_to,
            self._flt_payment_method_combo,
            self._flt_status_partial_combo,
            self._flt_status_full_combo,
            self._flt_sum_from,
            self._flt_sum_to,
            self._flt_client_phone,
            self._flt_client_inn,
        ):
        ...
        if self._flt_surcharge_from is not None:
            self._flt_surcharge_from.clear()
        if self._flt_surcharge_to is not None:
            self._flt_surcharge_to.clear()
    ...
    def _on_refresh_orders_list(self):
        ...
        if sel:
            it = self._orders_table.item(sel[0].row(), 0)
            if it:
                sel_ck = it.data(Qt.UserRole)
        self._skip_portal_snap_reuse = True
        try:
            self._load_orders(force_refresh=True)
        finally:
            self._skip_portal_snap_reuse = False
        if sel_ck:
            for row in range(self._orders_table.rowCount()):
                it = self._orders_table.item(row, 0)
                if it and it.data(Qt.UserRole) == sel_ck:
    ...
    def _init_orders_list_listen(self):
        """PostgreSQL LISTEN mirror_orders_list — обновление таблицы по факту изменений в БД."""
        self._orders_list_listen = None
        try:
            from ui.orders_list_pg_listen import OrdersListPgListen
        except Exception:
            return
        lst = OrdersListPgListen(self)
        self._orders_list_listen = lst
        lst.notify.connect(self._on_orders_list_pg_notify)
        lst.start()

    def _on_orders_list_pg_notify(self, payload: str):
        self._orders_notify_pending.add(str(payload or "refresh").strip() or "refresh")
        self._orders_notify_timer.stop()
        self._orders_notify_timer.start(self._orders_notify_ms)

    def _flush_orders_list_notify_batch(self):
        if not self._orders_notify_pending:
            return
        pending = set(self._orders_notify_pending)
        self._orders_notify_pending.clear()
        need_full = False
        mo_ins, mo_upd, mo_del = [], [], []
        ms_ins, ms_upd, ms_del = [], [], []
        bz_oids = set()
        pe_oids = set()
        client_touch = False
        for p in pending:
            if not p or p == "refresh":
                need_full = True
                break
            parts = p.split(":")
            if parts[0] == "mo" and len(parts) >= 3:
                op, sid = parts[1], parts[2]
                try:
                    oid = int(sid)
                except (TypeError, ValueError):
                    need_full = True
                ...
                if op == "insert":
                    ...
                    mo_upd.append(oid)
            elif parts[0] == "ms" and len(parts) >= 3:
                op, sid = parts[1], parts[2]
                try:
                    oid = int(sid)
                except (TypeError, ValueError):
                ...
                if op == "insert":
                    ...
                    ms_upd.append(oid)
            elif parts[0] == "bz" and len(parts) >= 2:
                try:
                    bz_oids.add(int(parts[1]))
                except (TypeError, ValueError):
                    need_full = True
                    break
            elif parts[0] == "pe" and len(parts) >= 3:
                try:
                    pe_oids.add(int(parts[2]))
                except (TypeError, ValueError):
                    need_full = True
                    break
            elif parts[0] == "cl":
                ...
                need_full = True
                break
        if need_full or len(pending) > 15:
            self._start_orders_list_background_reload(force_refresh=True)
            return
        tbl = getattr(self, "_orders_table", None)
        freeze = tbl is not None and getattr(self, "_main_table_mode", "") == "orders"
        if freeze:
            tbl.setUpdatesEnabled(False)
        try:
            for oid in mo_del:
            ...
            for oid in mo_ins:
                self._light_append_mirror_order_from_db(int(oid))
            for oid in mo_upd:
                self._light_resync_mirror_order_from_db(int(oid), refresh_items=True)
            for oid in ms_del:
                self._light_remove_sales_order_from_memory(int(oid))
            for oid in ms_ins:
                self._light_append_sales_order_from_db(int(oid))
            for oid in ms_upd:
                self._light_resync_sales_order_from_db(int(oid))
            for oid in pe_oids:
                if int(oid) not in mo_upd:
                    self._light_resync_mirror_order_from_db(int(oid), refresh_items=False)
            for oid in bz_oids:
                self._resync_portal_cells_for_order(int(oid))
            if client_touch:
                self._reload_clients_cache_for_orders_list()
        finally:
            if freeze:
                tbl.setUpdatesEnabled(True)
                tbl.viewport().update()

    def _start_orders_list_background_reload(self, force_refresh: bool = True):
        if getattr(self, "_orders_list_fetch_thread", None) is not None and self._orders_list_fetch_thread.isRunning():
            self._orders_list_fetch_pending = True
            return
        th = _OrdersListFetchThread(force_refresh, self)
        self._orders_list_fetch_thread = th
        th.snapshot_ready.connect(self._on_orders_list_snapshot_ready)
        th.finished.connect(th.deleteLater)
        th.start()

    def _on_orders_list_snapshot_ready(self, snap):
        self._orders_list_fetch_thread = None
        if isinstance(snap, dict):
            self._apply_orders_list_snapshot(snap)
        if getattr(self, "_orders_list_fetch_pending", False):
            self._orders_list_fetch_pending = False
            QTimer.singleShot(0, lambda: self._start_orders_list_background_reload(force_refresh=True))

    def _apply_orders_list_snapshot(self, snap: dict):
        """Применить снимок списка заказов на UI (без блокирующих запросов к БД)."""
        orders = list(snap.get("orders") or [])
        merged_orders = list(snap.get("merged_orders") or [])
        prev_calc = {}
        ...
        self._all_quick = snap.get("quick") or []
        clients = snap.get("clients") or []
        self._all_clients_for_search = list(clients)
        self._clients_by_id = {}
        self._clients_by_name = {}
        ...
        self._orders_items = dict(snap.get("orders_items") or {})
        self._order_cut_ids = snap.get("order_cut_ids") or frozenset()
        self._blocks_zamer_by_order = dict(snap.get("blocks_zamer_by_order") or {})
        self._production_events_by_order = dict(snap.get("production_events_by_order") or {})
        self._order_filter_cache = {}
        self._rebuild_order_row_meta_light()
        tbl = getattr(self, "_orders_table", None)
        if getattr(self, "_main_table_mode", "") == "orders" and tbl is not None:
            tbl.setUpdatesEnabled(False)
            try:
                self._table_display_order = self._orders_table_display_sequence()
                self._fill_orders_table(self._table_display_order)
    ...
    def _reload_clients_cache_for_orders_list(self):
        from db import models as db_models
        ...
        try:
            ...
            pass
        tbl = getattr(self, "_orders_table", None)
        if getattr(self, "_main_table_mode", "") != "orders" or tbl is None:
            return
        disp = getattr(self, "_table_display_order", None) or []
        tbl.setUpdatesEnabled(False)
        try:
    ...
    def _resync_portal_cells_for_order(self, order_id: int):
        from db import models as db_models
        ...
        row = self._table_row_for_cache_key(ck)
        if row < 0:
            return
        tbl = self._orders_table
        pr = None
        tbl.setUpdatesEnabled(False)
        try:
            tbl.setItem(row, _OC_MEASURE, self._orders_service_item(o, pr, "measure"))
            tbl.setItem(row, _OC_DELIVERY, self._orders_service_item(o, pr, "delivery"))
            tbl.setItem(row, _OC_INSTALL, self._orders_service_item(o, pr, "install"))
        finally:
            tbl.setUpdatesEnabled(True)
            tbl.viewport().update()
    ...
    def _orders_surcharge_item(self, o):
        ...
        if sur_lc and "(" in sur_lc:
            methods = sur_lc.split("(", 1)[-1].rstrip(")")
        txt = self._format_surcharge_short(paid, total, methods)
        it = QTableWidgetItem(txt)
        it.setToolTip(txt)
        return it
    ...
    def _orders_surcharge_cell(self, o):
        ...
        try:
            ...
            top.setText(
                self._format_surcharge_short(paid_amount, total_amount, methods_txt)
            )
    ...
    def showEvent(self, event):  # noqa: N802
        ...
        self._apply_orders_table_layout_resync()
    ...
    def changeEvent(self, event):  # noqa: N802
        ...
        if event.type() == QEvent.WindowStateChange:
            self._last_synced_section_widths = None
            QTimer.singleShot(0, self._apply_orders_table_layout_resync)
            QTimer.singleShot(100, self._apply_orders_table_layout_resync)
            QTimer.singleShot(250, self._apply_orders_table_layout_resync)
    ...
    def _load_orders(self, force_refresh: bool = False):
        """Полная перезагрузка списка заказов из БД (синхронно, при старте и явных действиях)."""
        try:
            snap = fetch_orders_list_snapshot(force_refresh=bool(force_refresh))
        except Exception:
            snap = None
        if isinstance(snap, dict):
            self._apply_orders_list_snapshot(snap)
    ...
    def _on_delivery_estimate(self):
        from ui.delivery_estimate_dialog import DeliveryEstimateDialog

        dlg = DeliveryEstimateDialog(self)
        dlg.exec_()
    ...
    def _download_cut_g_files(self, order_data):
        """Скачать ZIP со всеми .G файлами раскроя для стола PRAKTIK/Optima."""
        from PyQt5.QtWidgets import QMessageBox
        from mirror_cut_sys_path import mirror_cut_imports_first
        ...
        try:
            ...
            oid_int = 0
        if oid_int < 1:
            QMessageBox.warning(self, "G файл", "Заказ не найден.")
            return

        from db import models as db_models

        row = db_models.get_order(oid_int)
        if not row:
            QMessageBox.warning(self, "G файл", "Заказ не найден.")
            return
        if not order_status_allows_production_print(row.get("status")):
            QMessageBox.warning(
                self,
                "G файл",
                "Файлы для стола резки доступны только для оплаченных заказов "
                "(статус «Оплачен» и далее по цепочке производства).",
            )
            return

        with mirror_cut_imports_first():
            from logic.g_export import (
                build_g_files_zip_bytes,
                g_files_zip_filename,
                iter_g_files_for_layouts,
                order_info_from_row,
            )

            layouts = db_models.get_cut_layouts_for_overview(oid_int) or []
            info = order_info_from_row(row)
            files = iter_g_files_for_layouts(layouts, info)
            if not files:
                QMessageBox.warning(
                    self,
                    "G файл",
                    "Нет сохранённого раскроя для формирования G-файлов.",
                )
                return
            default_name = g_files_zip_filename(oid_int)
            cfg = app_cfg()
            folder = get_cfg_string(cfg, "paths", "cutting_pdf_dir", "") if cfg else ""
            if not folder:
                folder = get_base_dir()
            ...
            if not save_path.lower().endswith(".zip"):
                save_path += ".zip"
            try:
                data = build_g_files_zip_bytes(layouts, info)
                with open(save_path, "wb") as f:
                    f.write(data)
                _open_file(os.path.dirname(save_path))
            except Exception as e:
                QMessageBox.critical(self, "G файл", str(e))
