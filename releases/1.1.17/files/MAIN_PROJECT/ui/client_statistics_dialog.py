...
def _build_plotly_html(df, client_name: str, period_label: str) -> str:
    ...
    df = plotly_dt_column(df, "created_at")
...
class _ClientStatsLoadWorker(QThread):
    finished_ok = pyqtSignal(object, str)
    failed = pyqtSignal(str)

    def __init__(self, client_id, dt_from, dt_to_ex, label, parent=None):
    ...
    def run(self):
        try:
            mir = db_models.get_orders_by_client_id_in_range(
                self._client_id, self._dt_from, self._dt_to_ex
            )
            sal = db_models.list_sales_orders_for_client_id(
                self._client_id, self._dt_from, self._dt_to_ex
            )
            ...
            self.finished_ok.emit(df, self._label)
...
class ClientStatisticsDialog(QDialog):
    ...
    def _reload(self):
        ...
        self._loader = _ClientStatsLoadWorker(self._client_id, dt_from, dt_to_ex, label, self)
        self._loader.finished_ok.connect(self._on_stats_loaded)
        self._loader.failed.connect(self._on_stats_failed)
        self._loader.start()

    def _on_stats_loaded(self, df, label):
