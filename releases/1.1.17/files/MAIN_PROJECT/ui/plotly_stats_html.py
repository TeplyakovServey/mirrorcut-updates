...
def plotly_dt_series(series):
    """Pandas datetime → list of python datetime (Plotly без FutureWarning)."""
    out = []
    for x in series:
        if x is None:
            out.append(None)
        ...
        if hasattr(x, "to_pydatetime"):
            out.append(x.to_pydatetime())
        else:
            out.append(x)
    return out


def plotly_dt_column(df, col):
    """Заменить datetime-колонку на python datetime перед передачей в Plotly."""
    if df is None or col not in getattr(df, "columns", []):
        return df
    out = df.copy()
    s = out[col]
    dtype = str(getattr(s, "dtype", ""))
    if dtype.startswith("datetime") or hasattr(s, "dt"):
        out[col] = plotly_dt_series(s)
    return out
