...
class GlassOrderOverviewDialog(QDialog):
    ...
    def __init__(self, order_data, parent=None, *, summary_only=False):
        ...
        can_g_files = order_status_allows_production_print(order_status) and has_cut
        ...
        if not self._summary_only:
            ...
            btn_g = QPushButton("G файл")
            btn_g.clicked.connect(self._on_g_files)
            btn_g.setVisible(bool(can_g_files))
            if can_g_files:
                btn_g.setToolTip(
                    "Скачать ZIP со всеми G-файлами для стола резки PRAKTIK "
                    "(по одному файлу на каждый лист раскроя)."
                )
            if can_print:
    ...
    def _on_g_files(self):
        parent = self.parent()
        if parent is not None and hasattr(parent, "_download_cut_g_files"):
            parent._download_cut_g_files(self._order_data)
