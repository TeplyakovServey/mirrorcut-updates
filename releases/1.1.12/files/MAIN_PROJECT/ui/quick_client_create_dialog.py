...
_CREATE_BTN_QSS = (
    "QPushButton#quickClientCreateBtn {"
    "  background-color: #c8e6c9; color: #1b5e20; font-weight: 700;"
    "  padding: 8px 20px; border: 1px solid #81c784; border-radius: 6px;"
    "  min-height: 28px; font-size: 11px;"
    "}"
    "QPushButton#quickClientCreateBtn:hover { background-color: #a5d6a7; border-color: #66bb6a; }"
    "QPushButton#quickClientCreateBtn:pressed { background-color: #81c784; }"
)


def _cfg_color(section_key: str) -> str:
    try:
        from cfg_loader import color as cfg_color

        return cfg_color(section_key)
    except Exception:
        return {"main_window_bg": "#E8F4FC", "orders_area_bg": "#D4EBF7"}.get(
            section_key, "#E8F4FC"
        )


def _apply_quick_client_dialog_style(d: QDialog) -> None:
    """Голубая тема полей (без оранжевой подсветки маски телефона / стандартных кнопок)."""
...
def open_quick_client_create_dialog(
    ...
    *,
    ...
    _apply_quick_client_dialog_style(d)
    ...
    btn_create.setObjectName("quickClientCreateBtn")
    btn_create.setStyleSheet(_CREATE_BTN_QSS)
    btn_create.setDefault(True)
    btn_create.clicked.connect(d.accept)
