...
_MIRROR_SILVER_5_ENSURED = False
_MIRROR_SILVER_CANONICAL_VARIANT = "серебро бесцветное"
_MIRROR_SILVER_NORM_DONE = False


def _is_mirror_silver_colorless_variant(name: str) -> bool:
    t = (name or "").strip().lower().replace(" ", "")
    s = t.replace("\\", "/")
    if "серебро" not in s:
        return False
    if "бесцвет" in s:
...
def normalize_mirror_silver_variant_names() -> None:
    """Один канонический вариант «серебро бесцветное» вместо «серебро б\\цв» и др."""
    global _MIRROR_SILVER_NORM_DONE
    if _MIRROR_SILVER_NORM_DONE:
        return
    conn = get_raw_connection()
    if not conn:
        return
    canon = _MIRROR_SILVER_CANONICAL_VARIANT
    try:
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT material_variant, thickness, price, status_zakalka
                FROM materials
            ...
            alias_rows = []
            canon_th = {}
            for mv, th, price, st in rows:
                if (mv or "").strip() == canon:
                    canon_th[int(th or 0)] = (mv, th, price, st)
                elif _is_mirror_silver_colorless_variant(mv):
                    alias_rows.append((mv, th, price, st))
            for mv, th, price, st in alias_rows:
                th_i = int(th or 0)
                if th_i in canon_th:
                    cur.execute(
                        """
                        DELETE FROM materials
                        WHERE material_type = 'Зеркало' AND material_variant = %s AND thickness = %s
                        """,
                        (mv, th_i),
                    )
                else:
                    cur.execute(
                        """
                        UPDATE materials SET material_variant = %s
                        WHERE material_type = 'Зеркало' AND material_variant = %s AND thickness = %s
                        """,
                        (canon, mv, th_i),
                    )
                    canon_th[th_i] = (canon, th_i, price, st)
        conn.commit()
        _MIRROR_SILVER_NORM_DONE = True
    except Exception:
        try:
            conn.rollback()
        except Exception:
...
def ensure_mirror_silver_colorless_5mm(price_rub: int = 2100, status_zakalka: int = 0) -> None:
    """Зеркало «серебро бесцветное» 5 мм в прайсе materials (калькулятор и окно «Цены»)."""
    global _MIRROR_SILVER_5_ENSURED
    normalize_mirror_silver_variant_names()
    if _MIRROR_SILVER_5_ENSURED:
        return
    conn = get_raw_connection()
    if not conn:
        return
    variant = _MIRROR_SILVER_CANONICAL_VARIANT
    try:
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT 1 FROM materials
                WHERE material_type = 'Зеркало' AND material_variant = %s AND thickness = 5
                """,
                (variant,),
            )
            ex = cur.fetchone()
            if ex:
                cur.execute(
                    """
                    UPDATE materials SET price = %s, status_zakalka = %s
                    WHERE material_type = 'Зеркало' AND material_variant = %s AND thickness = 5
                    """,
                    (int(price_rub), int(status_zakalka), variant),
                )
            else:
                cur.execute(
                    """
                    INSERT INTO materials (material_type, material_variant, thickness, price, status_zakalka)
                    VALUES ('Зеркало', %s, 5, %s, %s)
                    """,
                    (variant, int(price_rub), int(status_zakalka)),
                )
        conn.commit()
        _MIRROR_SILVER_5_ENSURED = True
    except Exception:
        try:
            conn.rollback()
        except Exception:
...
def load_materials_tree() -> dict:
    ...
    """
    ensure_mirror_silver_colorless_5mm()
    conn = get_raw_connection()
    if not conn:
        return {}
    tree = {}
