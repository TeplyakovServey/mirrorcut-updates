...
from calc.geometry import minimal_bounding_box_dims_triangle
...
def compute_packaging_block(
    ...
    def _line(name: str, used: bool, per_m2: int) -> int:
        ...
        if not used:
            return 0
        per_one = int(math.ceil(area * per_m2))
        per_all = per_one * quantity
        details[name] = {
            "Используется": True,
    ...
    _line(
        "Стрейч-плёнка",
        stretch,
        _price_lookup(prices_lower, "стрейтч-пленка", "стрейч-плёнка", "stretch"),
    )
    _line(
