...
class Virez_Fame(QWidget):
    def __init__(self):
        ...
        self.items_list.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Expanding)
        self.items_list.setStyleSheet(
            "QListWidget { border: none; background: %s; color: %s; font-size: 9pt; }"
            "QListWidget::item { padding: 1px 2px; min-height: 14px; }"
            "QListWidget::item:selected { background: %s; color: #000; }"
            % (P.TILE_SURFACE, P.TILE_TEXT, P.CONTROL_ACCENT)
        )
        ...
        self.items_list.itemClicked.connect(lambda _i: self._sync_delete_button())
        self.items_list.installEventFilter(self)
        root.addWidget(self.items_list, 1)
        ...
        row_bottom.addWidget(self.add_button, 0)
        row_bottom.addWidget(self.btn_delete, 1)
    ...
    def _sync_delete_button(self) -> None:
        self.btn_delete.setEnabled(len(self.items) > 0)
    ...
    def add_item(self):
        ...
        self.items_list.setCurrentRow(len(self.items) - 1)
        self.amount.clear()
        self._sync_delete_button()
