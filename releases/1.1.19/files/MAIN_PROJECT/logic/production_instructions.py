...
def build_cut_g_file_bytes_for_order_sheet(order_row: Dict[str, Any], sheet_index: int) -> Tuple[str, bytes]:
    ...
    ctx = make_g_export_context(info)
    content = layout_to_g_content(lay, info, sheet_index=idx, batch_seq=1, g_ctx=ctx)
    from logic.g_export import _encode_g_content_cp1251
    return fname, _encode_g_content_cp1251(content)
