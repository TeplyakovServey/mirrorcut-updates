...
from calc.delivery_calc import (
    ...
    default_delivery_tier_code,
    delivery_base_rub_from_tier_code,
    delivery_tariff_payload,
)
...
class ZamerTile(QWidget):
    ...
    def __init__(self, client_strip, parent=None):
        ...
        self._delivery_tier_code = default_delivery_tier_code(None)

        # Три колонки сетки по ширине (раньше «замер» + отдельная плитка «доставка»).
        apply_service_tile_frame_fixed_width(self, grid_span_width(3))
        ...
        self._combo_delivery_tariff = QComboBox()
        self._combo_delivery_tariff.setFont(lf)
        for _code, _rub, label in DELIVERY_TIER_CHOICES:
            self._combo_delivery_tariff.addItem(label, _code)
        self._combo_delivery_tariff.setToolTip(
            "Базовая стоимость доставки в пределах КАД (и база за пределами КАД до пробега)"
        )
        self._combo_delivery_tariff.currentIndexChanged.connect(self._on_delivery_tariff_changed)
        left_col.addWidget(self._combo_delivery_tariff)
        ...
        self._visit_data = {
            ...
            **delivery_tariff_payload(self._delivery_tier_code),
        }
        ...
        self._sync_delivery_tariff_combo_from_code(self._delivery_tier_code)

    def _sync_delivery_tariff_combo_from_code(self, code: str) -> None:
        code = str(code or "").strip().lower()
        self._combo_delivery_tariff.blockSignals(True)
        for i in range(self._combo_delivery_tariff.count()):
            if self._combo_delivery_tariff.itemData(i) == code:
                self._combo_delivery_tariff.setCurrentIndex(i)
                break
        self._combo_delivery_tariff.blockSignals(False)

    def _on_delivery_tariff_changed(self, _idx: int = 0) -> None:
        data = self._combo_delivery_tariff.currentData()
        self._delivery_tier_code = str(data or default_delivery_tier_code(None))
        if isinstance(self._visit_data, dict):
            self._visit_data.update(delivery_tariff_payload(self._delivery_tier_code))
        self._refresh_visit_lbl()
        self.visitChanged.emit()

    def get_delivery_base_rub(self) -> int:
        return delivery_base_rub_from_tier_code(self._delivery_tier_code)

    def get_delivery_tariff_fields(self) -> dict:
        return delivery_tariff_payload(self._delivery_tier_code)
    ...
    def _on_client_strip_identity_changed(self) -> None:
        ...
        if cs is None:
            return
        try:
            pl = cs.get_payload() if hasattr(cs, "get_payload") else {}
            cid = pl.get("id") if isinstance(pl, dict) else None
            if cid:
            ...
            tier = None
        code = default_delivery_tier_code(tier)
        self._delivery_tier_code = code
        self._sync_delivery_tariff_combo_from_code(code)
        if isinstance(self._visit_data, dict):
            self._visit_data.update(delivery_tariff_payload(code))
        self.visitChanged.emit()
    ...
    def _on_visit_radio_changed(self):
        ...
        if self._rb_in.isChecked():
            ...
            tariff = delivery_tariff_payload(self._delivery_tier_code)
            self._visit_data = {
    ...
    def _apply_visit_ui(self):
        ...
        self.btn_map.setVisible(True)
        self.btn_map.setToolTip(
            "Открыть карту и указать адрес / маршрут (вне КАД — с расчётом км)"
        )
        if inside:
            addr = ""
            if isinstance(self._visit_data, dict):
    ...
    def _open_outside_dialog(self):
        if not self.has_any_service():
            return
        initial = dict(self._visit_data) if isinstance(self._visit_data, dict) else {}
        dlg = DeliveryOutsideDialog(
            initial or None,
            self,
            window_title="Адрес и маршрут",
        )
        if dlg.exec_() == QDialog.Accepted:
            self._visit_data = dlg.get_result()
            if bool(self._visit_data.get("Внутри КАД")):
                self._visit_mode = self.MODE_INSIDE
                self._rb_in.setChecked(True)
            else:
                self._visit_mode = self.MODE_OUTSIDE
    ...
    def to_selected_block(self) -> dict:
        visit = dict(self._visit_data) if isinstance(self._visit_data, dict) else {}
        visit.update(delivery_tariff_payload(self._delivery_tier_code))
        vd = {
    ...
    def apply_saved_block(self, blk) -> None:
        ...
        if isinstance(vd, dict) and vd:
            ...
            tier_code = vd.get("Тариф")
            if tier_code:
                self._delivery_tier_code = str(tier_code)
                self._sync_delivery_tariff_combo_from_code(self._delivery_tier_code)
