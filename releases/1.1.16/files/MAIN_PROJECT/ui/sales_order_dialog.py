...
class SalesOrderDialog(QDialog):
    ...
    def _sync_service_items_from_zamer(self):
        ...
        if zd.get("Доставка"):
            vd = zd.get("Данные выезда") if isinstance(zd.get("Данные выезда"), dict) else {}
            base = self.zamer_tile.get_delivery_base_rub() if hasattr(self.zamer_tile, "get_delivery_base_rub") else delivery_base_rub_from_data(vd, prices)
            _append_service("Доставка", delivery_price_rub(prices, inside, km, base_rub=base))
        if zd.get("Замер"):
    ...
    def _on_client_text(self, text):
        ...
        if not pref:
            ...
            return
        search_pref = (pref or "").strip()
        try:
            if self._quick_estimate_mode:
                sug = db_models.list_quick_estimate_client_suggestions(search_pref, limit=40) or []
                rows = sug
            else:
                rows = [
                    {"label": str(r.get("name") or "").strip(), "name": str(r.get("name") or "").strip(), "client_id": r.get("id"), "quick_client_id": None}
                    for r in (db_models.get_clients_by_prefix(search_pref) or [])
                    if r.get("name")
                ]
        except Exception:
            rows = []
