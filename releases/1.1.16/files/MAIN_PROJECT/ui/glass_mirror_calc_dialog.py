...
from window_branding import available_screen_geometry
...
class GlassMirrorCalcDialog(QDialog):
    def __init__(
        ...
        *,
        ...
        self._blocks_xx = None
        self._shared_pricing_cache = {}
        self._adding_product = False
        self._pending_initial = initial
        self._pending_product_id = str(product_id) if product_id else None
        self._pending_append_new = bool(append_new)
        self._pending_order_row = order_row

        lay = QVBoxLayout(self)
        lay.setContentsMargins(0, 0, 0, 0)
        lay.setSpacing(0)

        self._client_header = QWidget()
        self._client_header.setObjectName("glassCalcClientHeader")
        self._client_header.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)
        self._client_header_lay = QHBoxLayout(self._client_header)
        self._client_header_lay.setContentsMargins(4, 2, 4, 0)
        self._client_header_lay.setSpacing(6)

        # Без родителя до вставки в шапку — иначе на Windows кнопка становится отдельным окном.
        ...
        self._tab_widget.currentChanged.connect(self._on_calc_tab_changed)

        from ui.calc_summary_panel import CalcSummaryPanel

        self._calc_left = QWidget()
        self._calc_left.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Expanding)
        _left_lay = QVBoxLayout(self._calc_left)
        _left_lay.setContentsMargins(0, 0, 0, 0)
        _left_lay.setSpacing(0)
        _left_lay.addWidget(self._client_header, 0)
        _left_lay.addWidget(self._tab_widget, 1)

        self._calc_body = QWidget()
        _body_lay = QHBoxLayout(self._calc_body)
        _body_lay.setContentsMargins(0, 0, 0, 0)
        _body_lay.setSpacing(0)
        _body_lay.addWidget(self._calc_left, 0)
        self._tab_widget.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Expanding)
        self._summary_panel = CalcSummaryPanel(self)
        _body_lay.addWidget(self._summary_panel, 1)
        self._calc_body.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        lay.addWidget(self._calc_body, 1)
        ...
        try:
            ...
            self.setStyleSheet(
                ...
                "QDialog#mainGlassCalcDlg QTabWidget#glassCalcTabs::pane { border: none; background-color: %s; padding: 0; margin: 0; }"
    ...
    def _finish_deferred_init(self) -> None:
        ...
        if self._blocks_xx is None:
            import xx as blocks_xx  # noqa: WPS433 — каталог BLOCKS в sys.path

            self._blocks_xx = blocks_xx
        self._add_calc_tab(
            self._pending_initial,
            self._pending_product_id,
            self._pending_append_new,
            tab_title="Изделие 1",
        )
        self._sync_tab_bar_visibility()
        self._apply_tab_close_buttons()
        self._apply_order_client_to_first_tab(self._pending_order_row)
    ...
    def _on_calc_tab_changed(self, tab_index: int) -> None:
        self._refresh_summary_panel()

    def _all_main_apps(self):
        out = []
    ...
    def _refresh_summary_panel(self) -> None:
        panel = getattr(self, "_summary_panel", None)
        if panel is None:
            return
        panel.refresh_from_apps(self._all_main_apps(), self._tab_widget.currentIndex())

    def _wire_app_summary(self, app) -> None:
        if app is None or not hasattr(app, "calcSummaryChanged"):
            return
        try:
            app.calcSummaryChanged.disconnect(self._refresh_summary_panel)
        except Exception:
            pass
        app.calcSummaryChanged.connect(self._refresh_summary_panel)

    def _fit_dialog_to_calc(self) -> None:
        """Калькулятор слева фиксированной ширины; окно и панель «Расчёт» — на всю ширину экрана."""
        app = self._current_main_app()
        if app is not None:
            app._apply_compact_window_size()
        try:
            self.layout().activate()
            self._calc_body.layout().activate()
            tab_extra = 0
            try:
                tab_extra = self._tab_widget.tabBar().sizeHint().height()
            except Exception:
                pass
            calc_w = max(400, int(app.width()) if app is not None else 400)
            self._calc_left.setFixedWidth(calc_w)
            self._tab_widget.setFixedWidth(calc_w)
            self._client_header.setFixedWidth(calc_w)
            self._calc_body.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
            if getattr(self, "_summary_panel", None) is not None:
                self._summary_panel.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
            if app is not None:
                h = app.height() + tab_extra + int(self._client_header.sizeHint().height()) + 2
            else:
            ...
            if geo is not None:
                ...
                left = int(geo.x())
                top = int(geo.y())
                rh = max(300, min(int(h), int(geo.height())))
                y = top + max(0, (int(geo.height()) - rh) // 2)
                self.setMinimumSize(min(640, w), 300)
                self.setMaximumSize(16777215, 16777215)
                self.setGeometry(left, y, w, rh)
            else:
                screen = QGuiApplication.primaryScreen()
                avail = screen.availableGeometry() if screen else None
                avail_w = avail.width() if avail else 1200
                avail_h = avail.height() if avail else 800
                top_y = avail.y() if avail else 0
                left_x = avail.x() if avail else 0
                rh = max(300, min(int(h), int(avail_h)))
                self.setGeometry(left_x, top_y + max(0, (int(avail_h) - rh) // 2), int(avail_w), rh)
            if getattr(self, "_summary_panel", None) is not None:
                self._summary_panel._apply_collapsed_state()
        except Exception:
            pass
    ...
    def showEvent(self, event):  # noqa: N802
        ...
        if getattr(self, "_deferred_init_done", False):
            QTimer.singleShot(0, self._fit_dialog_to_calc)
    ...
    def _mount_shared_client_header(self) -> None:
        """Поле клиента — всегда вверху окна (не внутри вкладок)."""
        header = getattr(self, "_client_header", None)
        lay = getattr(self, "_client_header_lay", None)
        if header is None or lay is None:
            return
        app0 = self._main_app_at(0)
        if not app0 or not getattr(app0, "client_strip", None):
            return
        strip = app0.client_strip
        if strip.parent() is not header:
            old_parent = strip.parentWidget()
            if old_parent is not None and old_parent.layout() is not None:
                try:
                    old_parent.layout().removeWidget(strip)
                except Exception:
                    pass
            strip.setParent(header)
        while lay.count():
            item = lay.takeAt(0)
            w = item.widget()
            if w is not None and w is not strip:
                w.setParent(self)
        strip.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        lay.addWidget(strip, 1)
        if self._allow_add_product:
            lay.addWidget(self._btn_add_product, 0, Qt.AlignVCenter)
            self._btn_add_product.setVisible(True)
        elif getattr(self, "_btn_add_product", None) is not None:
            self._btn_add_product.hide()
        strip.show()
        for i in range(1, self._tab_widget.count()):
            app = self._main_app_at(i)
            if app and getattr(app, "client_strip", None):
                app.client_strip.hide()

    def _place_add_product_button(self, tab_index: int) -> None:
        """Кнопка «Добавить изделие» — в шапке клиента (всегда сверху)."""
        self._mount_shared_client_header()
    ...
    def _create_main_app(self, blocks_xx, *, initial_blocks_json, finish_cb):
        """MainApp: совместимость со старым xx.py после частичного дельта-обновления."""
        base_kw = dict(
        ...
        try:
            return blocks_xx.MainApp(
                **base_kw,
                shared_pricing_cache=self._shared_pricing_cache,
                defer_pricing_warm=True,
            )
        except TypeError:
            app = blocks_xx.MainApp(**base_kw)
            cache = getattr(self, "_shared_pricing_cache", None)
            if isinstance(cache, dict):
                app._pricing_cache = cache
            elif not hasattr(app, "_pricing_cache"):
                app._pricing_cache = {}
            if hasattr(app, "_warm_pricing_cache"):
                QTimer.singleShot(0, app._warm_pricing_cache)
            return app
    ...
    def _add_calc_tab(self, initial_blocks_json, product_id: str | None, append_new: bool, tab_title: str):
        ...
        lay_tab.setAlignment(Qt.AlignTop | Qt.AlignLeft)

        app = self._create_main_app(
            blocks_xx,
            initial_blocks_json=initial_blocks_json,
            finish_cb=finish_cb,
        )
        ...
        if n > 0:
            ...
            if pl:
                ...
                app.selected["Клиент"] = dict(pl)
                app._sync_glass_visibility_from_client()
    ...
    def _open_append_product(self):
        if not self._allow_add_product or self._adding_product:
            return
        app0 = self._main_app_at(0)
        if app0 is not None and hasattr(app0, "_require_client_resolved"):
            if not app0._require_client_resolved("Добавить изделие"):
                return
        self._adding_product = True
        self._btn_add_product.setEnabled(False)
        self._btn_add_product.setText("Создание вкладки…")
        QApplication.processEvents()
        QTimer.singleShot(0, self._deferred_append_product)

    def _deferred_append_product(self):
        try:
            n = self._tab_widget.count()
            self._add_calc_tab(
            ...
            self._adding_product = False
            self._btn_add_product.setText("Добавить изделие")
            self._btn_add_product.setEnabled(bool(self._allow_add_product))
