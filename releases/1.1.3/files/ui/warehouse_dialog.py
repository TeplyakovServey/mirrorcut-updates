...
def _sort_mirror_variants(variants):
    """Для зеркала: «серебро б/цв» первым в списке цветов."""
    ...
    def _rank(v):
        ...
        if "серебро" in s and ("б/цв" in s or "бцв" in s or "б\\цв" in (v or "").lower()):
            return (0, v.lower())
        return (1, v.lower())
...
class AddSheetDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        layout.addRow("Поставщик:", self._supplier_picker)
...
class WarehouseDialog(QDialog):
    def __init__(self, parent=None):
        ...
        full_tab_layout.addWidget(btn_add_sheets)
        view_row = QHBoxLayout()
        view_row.addWidget(QLabel("Вид:"))
        self._full_view_grp = QButtonGroup(self)
        self._btn_full_table = QPushButton("Таблица")
        self._btn_full_table.setCheckable(True)
        self._btn_full_table.setChecked(True)
        self._btn_full_tiles = QPushButton("Картинки")
        self._btn_full_tiles.setCheckable(True)
        self._full_view_grp.addButton(self._btn_full_table, 0)
        self._full_view_grp.addButton(self._btn_full_tiles, 1)
        self._full_view_grp.buttonClicked.connect(self._on_full_view_mode)
        view_row.addWidget(self._btn_full_table)
        view_row.addWidget(self._btn_full_tiles)
        view_row.addStretch()
        full_tab_layout.addLayout(view_row)
        self._full_stack = QStackedWidget()
        self._full_table_scroll = QScrollArea()
        self._full_table_scroll.setWidgetResizable(True)
        self._full_table_host = QWidget()
        self._full_table_layout = QVBoxLayout(self._full_table_host)
        self._full_table_layout.setAlignment(Qt.AlignTop)
        self._full_table_scroll.setWidget(self._full_table_host)
        self._full_stack.addWidget(self._full_table_scroll)
        self.full_scroll = QScrollArea()
        self.full_scroll.setWidgetResizable(True)
        self.full_content = QWidget()
        self.full_grid = QGridLayout(self.full_content)
        for c in range(TILES_PER_ROW):
            self.full_grid.setColumnStretch(c, 1)
        self.full_scroll.setWidget(self.full_content)
        self._full_stack.addWidget(self.full_scroll)
        self._full_stack.setCurrentIndex(0)
        full_tab_layout.addWidget(self._full_stack, 1)
        self._sheets_cache = []
        ...
        self.btn_delete_remnant = QPushButton("Удалить")
        self.btn_delete_remnant.setToolTip("Удалить остаток: выбор материала → клик по листу → подтверждение. Лист попадёт в архив удалённых.")
        self.btn_delete_remnant.clicked.connect(self._on_delete_remnant)
        rem_btn_row.addWidget(self.btn_delete_remnant)
        self.btn_deleted_archive = QPushButton("Архив удалённых")
        self.btn_deleted_archive.setToolTip("Просмотр таблицы удалённых остатков (размер, толщина, дата и время удаления)")
        ...
        rem_view_row = QHBoxLayout()
        rem_view_row.addWidget(QLabel("Вид:"))
        self._rem_view_grp = QButtonGroup(self)
        self._btn_rem_table = QPushButton("Таблица")
        self._btn_rem_table.setCheckable(True)
        self._btn_rem_table.setChecked(True)
        self._btn_rem_tiles = QPushButton("Картинки")
        self._btn_rem_tiles.setCheckable(True)
        self._rem_view_grp.addButton(self._btn_rem_table, 0)
        self._rem_view_grp.addButton(self._btn_rem_tiles, 1)
        self._rem_view_grp.buttonClicked.connect(self._on_rem_view_mode)
        rem_view_row.addWidget(self._btn_rem_table)
        rem_view_row.addWidget(self._btn_rem_tiles)
        rem_view_row.addStretch()
        rem_tab_layout.addLayout(rem_view_row)
        self._rem_stack = QStackedWidget()
        self._rem_table_scroll = QScrollArea()
        self._rem_table_scroll.setWidgetResizable(True)
        self._rem_table_host = QWidget()
        self._rem_table_layout = QVBoxLayout(self._rem_table_host)
        self._rem_table_layout.setAlignment(Qt.AlignTop)
        self._rem_table_scroll.setWidget(self._rem_table_host)
        self._rem_stack.addWidget(self._rem_table_scroll)
        self.rem_scroll = QScrollArea()
        self.rem_scroll.setWidgetResizable(True)
        self.rem_content = QWidget()
        self.rem_main_layout = QVBoxLayout(self.rem_content)
        self.rem_main_layout.setAlignment(Qt.AlignTop)
        self.rem_scroll.setWidget(self.rem_content)
        self._rem_stack.addWidget(self.rem_scroll)
        self._rem_stack.setCurrentIndex(0)
        rem_tab_layout.addWidget(self._rem_stack, 1)
    ...
    def _on_add_sheet(self):
        ...
        try:
            ...
            QMessageBox.information(self, "Добавлено", "Листы добавлены на склад.")
            self._load_full()
        except Exception as e:
            QMessageBox.critical(self, "Ошибка", "Не удалось сохранить: %s" % e)

    def _on_full_view_mode(self, btn):
        self._full_stack.setCurrentIndex(0 if btn is self._btn_full_table else 1)

    def _on_rem_view_mode(self, btn):
        self._rem_stack.setCurrentIndex(0 if btn is self._btn_rem_table else 1)
    ...
    def _load_full(self):
        ...
        self._render_full_tiles()

    def _render_full_table(self):
        self._clear_vbox_layout(self._full_table_layout)
        ...
        for (name, th) in sorted(buckets.keys(), key=lambda k: (k[0], k[1])):
            ...
            sublabel = QLabel("%s — %d мм" % (name or "—", th))
            sublabel.setStyleSheet("font-weight: bold; font-size: 12px; margin-top: 8px;")
            self._full_table_layout.addWidget(sublabel)
            tbl = QTableWidget()
            ...
            for (w, h), grp in sorted(by_size.items(), key=lambda x: -(x[0][0] * x[0][1])):
                ...
                rows.append((w, h, merged))
            tbl.setRowCount(len(rows))
            for ri, (w, h, row) in enumerate(rows):
                area = (w * h) / 1_000_000.0 if w > 0 and h > 0 else 0
                cost = float(row.get("cost") or 0)
                m2 = (cost / area) if area > 0 and cost > 0 else 0
                tbl.setItem(ri, 0, QTableWidgetItem("%d × %d мм" % (w, h)))
                tbl.setItem(ri, 1, QTableWidgetItem(str(row.get("quantity") or 0)))
                ad = row.get("arrival_date")
                ad_s = ad.strftime("%d.%m.%Y") if ad and hasattr(ad, "strftime") else "—"
                tbl.setItem(ri, 2, QTableWidgetItem(ad_s))
                ...
                if sid:
                    ...
                    f.setUnderline(True)
                    it_sup.setFont(f)
                tbl.setItem(ri, 3, it_sup)
                tbl.setItem(ri, 4, QTableWidgetItem("%.2f" % cost if cost else "—"))
                tbl.setItem(ri, 5, QTableWidgetItem("%.2f" % m2 if m2 else "—"))
                tbl.setItem(ri, 6, QTableWidgetItem(str(row.get("warehouse_number") or "—")))
                tbl.setItem(ri, 7, QTableWidgetItem(str(row.get("comment") or "—")[:80]))
                tbl.setItem(ri, 8, QTableWidgetItem(str(row.get("id") or "")))

            def _on_sup_click(r, c, t=tbl):
                if c != 3:
                    return
                it = t.item(r, c)
                if it:
            ...
            tbl.cellClicked.connect(_on_sup_click)
            self._full_table_layout.addWidget(tbl)

    def _render_full_tiles(self):
        buckets = defaultdict(list)
        for s in self._sheets_cache:
            key = (
                (s.get("name") or "").strip(),
                int(s.get("thickness_mm") or 4),
                int(s.get("width_mm") or 0),
                int(s.get("height_mm") or 0),
            )
            buckets[key].append(s)
        sorted_keys = sorted(buckets.keys(), key=lambda k: (k[0], k[1], k[2] * k[3]))
        layout = self.full_grid
        while layout.count():
        ...
        for i, key in enumerate(sorted_keys):
            group = buckets[key]
            name, th, w, h = key
            total_qty = sum(int(x.get("quantity") or 0) for x in group)
            merged = dict(group[0])
            merged["quantity"] = total_qty
            merged["thickness_mm"] = th
            merged["_full_sheet_group_ids"] = [int(x["id"]) for x in group if x.get("id") is not None]
            display_name = name
            if total_qty != 1:
                display_name = "%s  ×%d" % (name, total_qty)
            tile = FullSheetStockTile(
                display_name, w, h, merged, self, on_hold_delete=self._on_full_sheet_hold_delete
            )
            row, col = i // TILES_PER_ROW, i % TILES_PER_ROW
            layout.addWidget(tile, row, col)
            if layout.rowStretch(row) == 0:
    ...
    def _load_remnants(self):
        ...
        self._render_rem_tiles()

    def _render_rem_table(self):
        self._clear_vbox_layout(self._rem_table_layout)
        by_key = defaultdict(list)
        for r in self._remnants_cache:
            name = r.get("name") or ""
            th = int(r.get("thickness_mm") or 4)
            area = (r.get("height_mm") or 0) * (r.get("width_mm") or 0)
            by_key[(name, th)].append((area, r))
        for key in by_key:
            by_key[key].sort(key=lambda x: -x[0])
        for (name, thickness_mm) in sorted(by_key.keys(), key=lambda k: (k[0], k[1])):
            items = [r for _, r in by_key[(name, thickness_mm)]]
            sublabel = QLabel("%s — %d мм" % (name if name else "Без названия", thickness_mm))
            sublabel.setStyleSheet("font-weight: bold; font-size: 12px; margin-top: 8px;")
            self._rem_table_layout.addWidget(sublabel)
            tbl = QTableWidget()
            tbl.setColumnCount(5)
            tbl.setHorizontalHeaderLabels(["№", "Размер", "Площадь м²", "Толщина", "Уник. №"])
            tbl.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
            tbl.setEditTriggers(QAbstractItemView.NoEditTriggers)
            ...
            for i, r in enumerate(items):
                ...
                tbl.setItem(i, 0, QTableWidgetItem(str(r.get("label_number") or r.get("unique_number") or "—")))
                tbl.setItem(i, 1, QTableWidgetItem("%d × %d мм" % (w, h)))
                tbl.setItem(i, 2, QTableWidgetItem("%.4f" % area))
                tbl.setItem(i, 3, QTableWidgetItem("%d" % thickness_mm))
                tbl.setItem(i, 4, QTableWidgetItem(str(r.get("unique_number") or "—")))
            self._rem_table_layout.addWidget(tbl)

    def _render_rem_tiles(self):
        by_key = defaultdict(list)
        for r in self._remnants_cache:
            name = r.get("name") or ""
            th = int(r.get("thickness_mm") or 4)
        ...
        for key in by_key:
            by_key[key].sort(key=lambda x: -x[0])
        while self.rem_main_layout.count():
            item = self.rem_main_layout.takeAt(0)
            if item.widget():
                item.widget().deleteLater()
            elif item.layout():
                self._clear_vbox_layout(item.layout())
        for (name, thickness_mm) in sorted(by_key.keys(), key=lambda k: (k[0], k[1])):
            items = [r for _, r in by_key[(name, thickness_mm)]]
            sublabel = QLabel("%s — %d мм" % (name if name else "Без названия", thickness_mm))
            sublabel.setStyleSheet("font-weight: bold; font-size: 12px; margin-top: 8px;")
            self.rem_main_layout.addWidget(sublabel)
            grid = QGridLayout()
            for c in range(REMNANT_TILES_PER_ROW):
            ...
            for i, r in enumerate(items):
                ...
                tile = GlassRemnantStockTile(
                    ...
                    dict(r),
                    self,
                    on_open_history=self._on_remnant_click,
                    on_hold_delete=self._on_remnant_hold_delete,
                )
                row, col = i // REMNANT_TILES_PER_ROW, i % REMNANT_TILES_PER_ROW
                grid.addWidget(tile, row, col)
                if grid.rowStretch(row) == 0:
                    grid.setRowStretch(row, 0)
            grid_w = QWidget()
            grid_w.setLayout(grid)
            self.rem_main_layout.addWidget(grid_w)
    ...
    def _on_profile_hold_delete(self, row):
        ...
        if sid is None:
            return
        try:
            if models.delete_profile_stock_and_archive(int(sid)):
                self._load_profile_full()
                self._load_profile_remnants()
            else:
    ...
    def _on_full_sheet_hold_delete(self, sheet):
        ...
        try:
            if gids:
                for sid in gids:
                    models.delete_full_sheet(int(sid))
            else:
                sid = sheet.get("id")
                if sid is None:
                    return
    ...
    def _on_show_deleted_archive(self):
        rows = models.get_deleted_remnants(500)
        d = QDialog(self)
        d.setWindowTitle("Архив удалённых остатков")
        ...
        if not rows:
            ...
            text.append("Размер (мм)\tТолщина\tМатериал\tУдалил\tДата и время")
            text.append("")
            for r in rows:
                w = r.get('width_mm') or 0
                h = r.get('height_mm') or 0
                th = r.get('thickness_mm') or ''
                ...
                if dt:
                    ...
                    dt_str = '—'
                text.append("%d×%d\t%s\t%s\t%s\t%s" % (w, h, th, name, who, dt_str))
        lbl = QLabel("\n".join(text))
...
class DeleteProfileStockDialog(QDialog):
    ...
    def _on_delete(self, rid):
        if QMessageBox.question(self, "Подтверждение", "Удалить запись со склада?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No) != QMessageBox.Yes:
            return
        if models.delete_profile_stock_and_archive(rid):
            if self.on_deleted:
                self.on_deleted()
            self._fill()
...
class ProfileDeletedArchiveDialog(QDialog):
    def __init__(self, parent=None):
        ...
        if not rows:
            ...
            text.append("Тип | Название | Цвет | Длина(мм) | Кол-во | Дата удаления")
            text.append("")
            for r in rows:
                dt = r.get('deleted_at')
