...
# Длины полей (ИНН везде 12 цифр)
INN_LEGAL_LEN = 12
INN_IP_LEN = 12
INN_INDIVIDUAL_LEN = 12
KPP_LEN = 9
OKPO_LEN = 8
OGRN_LEN = 13
...
class NewClientDialog(QDialog):
    ...
    def __init__(
        ...
        self.inn_edit.setPlaceholderText("12 цифр, обязательно")
        self.inn_edit.setMaxLength(12)
        self.inn_edit.setValidator(_digits_validator(12))
        self.main_form.addRow("ИНН (12 цифр, обяз.):", self.inn_edit)
        ...
        self.inn12_edit.setPlaceholderText("12 цифр")
        self.inn12_edit.setMaxLength(12)
        self.inn12_edit.setValidator(_digits_validator(12))
        self.doc_ind_layout.addRow("ИНН (12 цифр):", self.inn12_edit)
    ...
    def _on_type_changed(self):
        ...
        if t == TYPE_LEGAL:
            ...
            self.main_form.labelForField(self.inn_edit).setText("ИНН (12 цифр):")
    ...
    def _validate_legal(self):
        ...
        if not name:
            ...
            return False
        if len(inn) != INN_LEGAL_LEN:
            QMessageBox.warning(self, "Ошибка", "ИНН юридического лица — 12 цифр.")
            return False
        if len(kpp) != KPP_LEN:
    ...
    def _validate_ip(self):
        ...
        if not name:
            ...
            return False
        if len(inn) != INN_IP_LEN:
            QMessageBox.warning(self, "Ошибка", "ИНН ИП — 12 цифр.")
            return False
        if not reg:
    ...
    def _validate_individual(self):
        ...
        if not first:
            ...
            return False
        if len(inn) != INN_INDIVIDUAL_LEN:
            QMessageBox.warning(self, "Ошибка", "ИНН физического лица — 12 цифр.")
            return False
