...
def _bundle_products_subtotal_and_services_rub(
    ...
    *,
    ...
    dlv, zam = _order_level_amounts_once_priced_mixed(
        products, conn, drill, markup_factor, delivery_prices, facade_aux_cached
    )
    return int(sub_products), int(dlv) + int(zam)
...
def _facade_service_money_from_payload(
    ...
    if dblk.get("Активирован") and isinstance(dblk.get("Данные"), dict):
        ...
        if inside or km is not None:
            dr = int(delivery_price_rub(prices, inside, km) or 0)
            if dr:
    ...
    if zblk.get("Активирован") and isinstance(zd, dict):
        ...
        if need_visit and (inside or km is not None):
            zr = int(delivery_price_rub(prices, inside, km) or 0)
            if zr:
                ...
                if zd.get("Монтаж"):
                    bits.append("монтаж")
                note = ", ".join(bits) if bits else "выезд"
                if addr:
                    note = "%s; %s" % (note, addr)
...
def _order_level_amounts_once_priced_mixed(
    ...
    """Первые ненулевые «Доставка» и «Замер (выезд)» по порядку изделий — в тех же рублях, что смета в этом окне."""
    dlv, zam = 0, 0
...
def _overview_bundle_order_total_rub(
    ...
    *,
    ...
    return int(sub_ex) + int(dlv) + int(zam)
    ...
    def _maybe_promote_order_made_after_bundle_save(self):
        ...
        try:
            ...
            if not row:
                return
            st = str(row.get("status") or "").strip().lower() or "draft"
            if st in ("made", "checked_qr", "shipped", "completed", "cancelled"):
                return
            if st == "draft":
                return
            fallback = st
