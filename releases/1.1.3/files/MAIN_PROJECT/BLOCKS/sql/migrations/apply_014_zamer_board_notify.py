#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Применить 014_zamer_board_notify.sql к PostgreSQL."""
...
def _sql_statements(sql_text: str) -> list[str]:
    """Разбиение по `;` в конце строки только вне тела `$$ ... $$` (PL/pgSQL)."""
    parts: list[str] = []
    buf: list[str] = []
    dollar_depth = 0
...
def main() -> int:
    path = os.path.join(_HERE, "014_zamer_board_notify.sql")
    ...
    try:
        ...
        for stmt in _sql_statements(text):
            with conn.cursor() as cur:
                cur.execute(stmt)
        conn.commit()
        print("OK: 014_zamer_board_notify")
        return 0
