...
def _cities_db_path() -> str:
    ...
    return os.path.join(_mirror_cut_root(), "CALC_WINDOWS", "cities_streets.db")
...
class DeliveryOutsideDialog(QDialog):
    ...
    def _on_ok(self):
        ...
        if kad_contains(self._lon, self._lat):
            QMessageBox.warning(
                self,
                "КАД",
                "Точка внутри КАД. Закройте окно и на плитке выберите «В пределах КАД».",
            )
            return
    ...
    def get_result(self) -> dict:
        return {
            ...
            "Внутри КАД": False,
            "Расстояние до КАД": self._distance_km,
