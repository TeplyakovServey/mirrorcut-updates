...
from elements.calc_tile_style import glass_tile_children_qss
...
# 2 плитки 200×200 + зазор 5 между ними (как в сетке xx.py)
TILE = 200
GAP = 5
BIG_SIDE = 2 * TILE + GAP
SKETCH_BOX = 76
...
class GlassProductTile(QFrame):
    ...
    def __init__(self, parent=None, *, show_additional_button: bool = True):
        ...
        grid.addWidget(self.btn_calc, 12, 0, 1, 4)
        ...
        grid.addWidget(self.btn_more, 13, 0, 1, 4)
    ...
    def _populate_materials(self):
        ...
        if self._use_db:
            mats = [m for m in sorted(self._mat_tree.keys()) if m not in HIDDEN_MATERIAL_TYPES]
            self.combo_material.addItems(mats)
        else:
            mats = [m for m in list(materials_dict.keys()) if m not in HIDDEN_MATERIAL_TYPES]
            self.combo_material.addItems(mats)
