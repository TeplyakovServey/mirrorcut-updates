...
ORDER_LEVEL_ROW_NAMES = frozenset({"Доставка", "Замер (выезд)"})
...
def order_level_amounts_once(payloads: List[Dict[str, Any]]) -> Tuple[int, int]:
    """Первое ненулевое Доставка и Замер (выезд) по порядку изделий — один раз на заказ."""
    dlv, zam = 0, 0
    for sel in payloads:
...
def bundle_grand_total_rub(products: List[Dict[str, Any]]) -> int:
    ...
    dlv, zam = order_level_amounts_once(glass_payloads)
    return sub + dlv + zam + extra
...
_ORDER_UNIT_TERMINAL_STATUSES = frozenset({"made", "checked_qr", "shipped", "completed"})
    ...
    for idx, pr in enumerate(products or []):
        ...
        if kind == "facade" and oid is not None and count_facade_assembled is not None:
            ...
            if int(q) > 1:
                idle_st = (
                    pst
                    if pst not in ("made", "checked_qr", "shipped", "completed")
                    else "in_progress"
                )
            ...
            st = (
                ...
                if (
                    done_n >= 1
                    or pst in ("made", "checked_qr", "shipped", "completed")
                )
                else pst
            )
            counts[st] = int(counts.get(st) or 0) + 1
        ...
        if kind != "facade" and pgs == PRODUCTION_GLASS_MADE:
            ...
            if pst_l in ("checked_qr", "shipped", "completed", "cancelled"):
